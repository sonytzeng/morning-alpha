...
## 7. P1 唯讀監控骨架（未 Migration / 未 Deploy）
...
四表均啟用並強制 RLS；未建立 anon/authenticated policy，並撤銷兩者的 table privileges。寫入預期只由 server-side service-role 執行。`ma_ops_recovery_actions` 在 P1 只有 audit schema，沒有 executor、trigger、Cron 或 retry；registry 沒有 seed。

P1 request 採 `{environment, check_type, target_date, dry_run, components?, request_id?}`；target date 預設為 Asia/Taipei 今日。成功 response 採 `{ok, run_id, environment, check_type, target_date, status, severity, summary, checks, recovery_attempted:false, generated_at}`。非 dry-run 且提供 `request_id` 時，以 `maops:p1:<environment>:<check_type>:<target_date>:<request_id>` 防重。

已建立 checks：market data freshness、daily report exists/date/contract、opening radar exists/freshness、War Room contract、closing verification status、Performance RPC availability、public site synthetic config。Public site 在 P1 只驗 URL 設定，不發 HTTP。LINE delivery 因尚無已驗證 audience baseline 與 delivery natural key，明確回 `skipped`。

P0 草案的 `organization_id`、短效簽章、registry-driven SLA、通知與 recovery guard 仍是 P2+ 設計，未納入本次指定的 P1 table/request contract。P1 不包含 `ma-ops-safe-recovery`，也未執行 Migration 或 Deploy。

## 8. P1.1 一致性修正

內部 `x-cron-secret` 與 runtime secret 各自做 SHA-256，再以固定長度 byte XOR 比較；缺失、空值或原始長度不同皆拒絕，secret/digest 不進入 log、metadata 或 response。

Read-only checks 保留 client timeout，late completion 沒有資料副作用。三個 audit writes（run insert、checks insert、run final update）直接等待 Supabase response，不使用會自行造成 unknown commit state 的 `Promise.race`。Run 建立後以統一 persistence try/catch/finally 收斂；後續寫入失敗時盡力標記 `failed`。平台層 request/DB timeout 後以 idempotency key 查證，不假設未提交。

同 key completed run 回既有結果與 `IDEMPOTENT_RESULT_REUSED`；running run 在 checks 前回 HTTP 202、`in_progress=true`、`IDEMPOTENT_RUN_IN_PROGRESS`，不建立第二筆。Insert unique race 會再次查詢並套用相同 contract。無 request ID 或 dry-run 不啟用重用。
