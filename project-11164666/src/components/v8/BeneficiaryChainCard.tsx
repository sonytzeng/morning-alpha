  ...
  return (
    <section className={`rounded-2xl p-5 md:p-6 ${cardClass}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className={`text-[10px] uppercase tracking-[0.3em] font-semibold ${dark ? 'text-emerald-300' : 'text-emerald-700'}`}>V8 Beneficiary Chain</p>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {beneficiaries.map((item, idx) => (
              <article key={`${item.symbol}-${idx}`} className={`rounded-xl p-4 ${panelClass}`}>
                ...
                <div className="mt-3">
                  <p className={`mb-1 text-[10px] uppercase tracking-wider ${mutedClass}`}>reason_chain</p>
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  <div>
                    <p className={`text-[10px] uppercase tracking-wider ${mutedClass}`}>risk_level</p>
                    <p className="text-xs font-semibold">{text(item.risk_level)}</p>
                  </div>
                  <div>
                    <p className={`text-[10px] uppercase tracking-wider ${mutedClass}`}>invalidation</p>
