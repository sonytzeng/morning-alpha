...
export function buildLineDailyFlexMessage(input) {
  const marketOnly = input.decisionMode === 'market_only';
  const recommendations = marketOnly ? [] : normalizeRecommendations(input.recommendations);
  ...
  const focusContents = focus.length > 0
    ...
    : [{
      type: 'text',
      text: marketOnly ? firstText(input.recommendationMessage, '推薦評估證據不足，今日暫不發布正式個股推薦')
        : decisionMode === 'no_trade' ? '先驗證盤面，不為了湊數勉強推薦。' : clipText(theme, 52),
      size: 'sm',
      color: '#425466',
      wrap: true,
      margin: 'sm',
    ...
    contents: {
      ...
      footer: {
        ...
        contents: [
          ...
          { type: 'text', text: marketOnly ? '完整市場證據與盤中驗證都在報告內' : '完整證據、5 檔排序與盤中驗證都在報告內', size: 'xxs', color: '#8B99A6', align: 'center', wrap: true, margin: 'sm' },
        ],
      },
    },
  };
