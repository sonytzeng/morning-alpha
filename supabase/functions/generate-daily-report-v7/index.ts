...
function buildV10BeneficiaryPhase1(candidateUniverse:Record<string,unknown>,evidenceIndex:V10EvidenceItem[]):V10BeneficiaryPhase1Output{const candidates=Array.isArray(candidateUniverse.candidates)?candidateUniverse.candidates as V10CandidateCard[]:[];const eligible=candidates.filter((candidate)=>candidate.eligibility!==false);const primaryEvent=String(candidateUniverse.primary_event??'UNKNOWN');const scored=eligible.map((candidate)=>calculateV10BeneficiaryPhase1Record(candidate,evidenceIndex,primaryEvent)).sort((a,b)=>b.total_score-a.total_score||b.evidence_score-a.evidence_score||a.repeat_penalty-b.repeat_penalty).map((record,index)=>({...record,rank:index+1}));const toOutputRecord=(record:V10BeneficiaryPhase1Record)=>({symbol:record.symbol,name:record.name,industry_code:record.industry_code,industry_name:record.industry_name,rank:record.rank,total_score:record.total_score,confidence_level:record.confidence_level,directional_event_score:record.directional_event_score,positive_evidence_count:record.positive_evidence_count,negative_evidence_count:record.negative_evidence_count,net_evidence_direction:record.net_evidence_direction,risk_flags:record.risk_flags,benefit_chain:record.benefit_chain,scoring_reasons:record.scoring_reasons,data_quality_status:record.data_quality_status});const riskWatchlist=scored.filter((record)=>record.net_evidence_direction==='negative').slice(0,10).map(toOutputRecord);const observationWatchlist=scored.filter((record)=>record.data_quality_status==='sufficient'&&record.net_evidence_direction==='neutral').slice(0,10).map(toOutputRecord);const positiveBacked=scored.filter((record)=>record.data_quality_status==='sufficient'&&record.net_evidence_direction==='positive'&&record.positive_evidence_count>=1&&record.positive_evidence_count>record.negative_evidence_count&&['medium','high'].includes(record.confidence_level)&&record.total_score>=45);const noCandidates=scored.length===0;const noPositiveEvidence=!noCandidates&&positiveBacked.length===0;const insufficient=noCandidates;const diverseTop=buildV10DiverseTopCandidates(positiveBacked,5).map((record,index)=>({...record,rank:index+1}));const today=insufficient||noPositiveEvidence?[]:diverseTop.map(toOutputRecord);const status=insufficient?'insufficient':noPositiveEvidence?'insufficient_positive_evidence':(today.length<5?'partial':'sufficient');const warning=insufficient?'今日事件、產業或行情驗證資料不足，未輸出 V10 強受惠股':noPositiveEvidence?'今日沒有足夠正向證據支持強受惠股，僅提供觀察名單與風險名單':null;const validation=validateV10BeneficiaryPhase1(scored.slice(0,10),candidateUniverse);return {enabled:false,candidate_count:scored.length,data_quality_status:status,warning,top_10_candidates:scored.slice(0,10),today_beneficiary_stocks_v10:today,observation_watchlist:observationWatchlist,risk_watchlist:riskWatchlist,validation};}
