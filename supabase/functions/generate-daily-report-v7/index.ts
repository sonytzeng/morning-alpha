...
function applyV10EvidenceBackedNarrative(ai:Record<string,unknown>,phase:V10BeneficiaryPhase1Output):Record<string,unknown>{
  ...
  if(recommendation){
    ...
    const sourceDetail=v10NarrativeText(logicSource[0]?.split('｜')[2]?.split('：')[0]||'隔夜證據',18);
...
function isSyntheticDailySentence(value:string):boolean{const text=value.replace(/\s+/g,' ').trim();if(!text)return true;return /基準日期|信心\s*\d+\s*\/\s*100/.test(text)||/^(偏多|偏空|偏弱|偏強|中性|震盪)觀察[，,。\s]/.test(text);}
