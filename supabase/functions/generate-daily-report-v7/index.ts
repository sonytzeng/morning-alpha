...
const VERSION='V9.4_CANONICAL_CATALYST_DECISION_REPAIR';
...
function normalizeV10MarketSnapshot(evidencePack:V10EvidencePack):Record<string,unknown>[]{const snapshot=(evidencePack.market_snapshot&&typeof evidencePack.market_snapshot==='object'?evidencePack.market_snapshot:{} ) as Record<string,unknown>;const defs=[['spx','SPX','index','US',82,60,'美股大盤風險偏好'],['nasdaq','NASDAQ','index','US',90,85,'科技股風險偏好'],['djia','DJIA','index','US',60,35,'美股傳統大型股風向'],['sox','SOX','index','US',98,96,'半導體族群對台股電子權值影響'],['tsm_adr','TSM','equity','US',96,98,'台積電 ADR 對 2330 與台股電子開盤情緒影響'],['nvda','NVDA','equity','US',94,92,'AI server 與半導體供應鏈風向'],['aapl','AAPL','equity','US',65,45,'大型科技與消費電子供應鏈情緒'],['msft','MSFT','equity','US',70,50,'AI 與雲端資本支出風向'],['meta','META','equity','US',60,45,'AI capex 與大型科技風險偏好'],['amzn','AMZN','equity','US',58,38,'雲端與大型科技風險偏好'],['vix','VIX','volatility','US',88,70,'市場風險溫度'],['dxy','DXY','fx','US',75,62,'美元強弱與外資資金壓力'],['us10y','US10Y','rate','US',78,68,'美債殖利率與估值壓力'],['crude_oil','CRUDE OIL','commodity','GLOBAL',55,35,'能源成本與通膨預期'],['taiex','TAIEX','index','TW',100,100,'台股現貨大盤方向'],['txf','TXF','futures','TW',95,95,'台指期與盤前/盤中風險方向'],['2330','2330','equity','TW',100,100,'台股最大權值與半導體核心驗證股']];return defs.map((def)=>{const [key,symbol,assetType,market,importance,taiwanRelevance,description]=def;const raw=snapshot[String(key)];if(!raw||typeof raw!=='object')return null;const r=raw as Record<string,unknown>;const changePercent=Number(r.change_percent);if(!Number.isFinite(changePercent)||!r.updated_at)return null;return {symbol,asset_type:assetType,market,value:r.value??null,direction:v10UpperDirection(r.direction),change_percent:changePercent,importance,taiwan_relevance:taiwanRelevance,description,raw_source:String(r.source??'market_data'),provider:r.provider??null,updated_at:r.updated_at,freshness:r.freshness_status??'unknown',quality_status:'verified'};}).filter(Boolean) as Record<string,unknown>[];}
...
async function fetchMarketData(supabase:ReturnType<typeof createClient>,log:(msg:string)=>void){
  ...
  try{
    const r=await supabase.from('market_data').select('*').order('captured_at',{ascending:false}).limit(30);
    const{data,error}=safeUnwrap<Record<string,unknown>[]>(r,log,'market_data');
    if(error||!data?.length){log('fmData:empty');return empty;}
    const normalized=normalizeMarketDataRows(data,Date.now(),HOURS_24);
...
Deno.serve(async (req:Request)=>{
  ...
  try{
    ...
    const rawDatesFetch=(async function(){try{const rr=await supabase.from('market_data').select('symbol,captured_at,value,change_percent').order('captured_at',{ascending:false}).limit(30);const{data}=safeUnwrap<Record<string,unknown>[]>(rr,log,'rawForDates');return normalizeMarketDataRows(data||[]).marketData.map(function(row){return{symbol:row.symbol,captured_at:row.updatedAt};});}catch{log('rawForDates fetch failed');return [] as Record<string,unknown>[];}})();
