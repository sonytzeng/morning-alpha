...
function attachResearchMasterV2Shadow(target:Record<string,unknown>,input:ResearchMasterV2AssemblerInput,log:(message:string)=>void):void{
  try{
    let publicationInput=input;
    if(input.isTradingDay && Array.isArray(target.today_beneficiary_stocks_v10)){
    ...
    const master=assembleResearchMasterV2(publicationInput);
    ...
    const researchDocument={...master,quality:validation.quality};
...
type MarketIndicator=NormalizedMarketIndicator;
type MarketNewsItem={id:string;title:string;source:string;url:string;published_at:string|null;created_at:string;related_sectors:string[]|null;related_symbols:string[]|null;event_type:string|null;taiwan_impact_summary:string|null;raw_payload:Record<string,unknown>|null};
...
type SectorRotationRow={sector:string;sub_sector:string;rotation_score:number;direction:string;signal_label:string;leading_symbols:string[];lagging_symbols:string[];summary?:string;score_date?:string};
type ResearchSectorRotationRow=SectorRotationRow&{lineage?:SectorEvidenceLineage};
function readSectorEvidenceLineage(row:SectorRotationRow|undefined):SectorEvidenceLineage|undefined{
  return (row as ResearchSectorRotationRow|undefined)?.lineage;
}
...
type V10EvidenceItem={evidence_id:string;evidence_type:string;source:string;title:string;summary:string;importance:number;freshness:string;raw_reference:string;published_at?:string|null;url?:string;quality_status?:'verified'|'context'};
...
function hasUsableMarketEvidence(m:MarketIndicator):boolean{return Boolean(m.hasValue&&m.hasChangePercent&&Number.isFinite(m.value)&&Number.isFinite(m.changePercent));}
function findIndicator(md:MarketIndicator[],syms:string[]):MarketIndicator|null{for(const sy of syms){const x=md.find(function(m){return m.symbol.toUpperCase()===sy.toUpperCase()&&hasUsableMarketEvidence(m)});if(x)return x}return null;}
...
function v10Indicator(md:MarketIndicator[],syms:string[]):Record<string,unknown>|null{const m=findIndicator(md,syms);return m?normalizePremiumMarketEvidence(m):null;}
...
function buildV10OvernightMoves(md:MarketIndicator[]):Record<string,unknown>[]{const symbols=[['SPX'],['IXIC','NASDAQ'],['SOX'],['DJIA'],['NVDA'],['TSM'],['AAPL'],['MSFT'],['META'],['AMZN'],['VIX'],['DXY'],['US10Y'],['CL','CRUDE_OIL','WTI'],['TAIEX'],['TXF'],['2330']];return symbols.map((syms)=>findIndicator(md,syms)).filter(Boolean).map((m)=>{const normalized=normalizePremiumMarketEvidence(m!);if(!normalized)return null;return {...normalized,importance:['SOX','IXIC','NASDAQ','NVDA','TSM','VIX','TXF','TAIEX','2330'].includes(String(m!.symbol).toUpperCase())?'core':'context',why_it_matters_for_taiwan:v10WhyTaiwanMatters(String(m!.symbol))};}).filter(Boolean) as Record<string,unknown>[];}
function buildV10NewsClusters(newsData:MarketNewsItem[]):Record<string,unknown>[] {
  const clusters=new Map<string,{topic:string;event_type:string;items:MarketNewsItem[];symbols:Set<string>;sectors:Set<string>}>();
  for(const item of newsData){
    const review=reviewPremiumNewsEvidence(item);
    if(!review.eligible)continue;
    const raw=[item.title,item.taiwan_impact_summary].filter(Boolean).join(' ');
    const eventType=review.event_type;
    const topic=item.related_sectors?.[0]??eventType;
    const key=`${eventType}:${topic}`;
    if(!clusters.has(key))clusters.set(key,{topic,event_type:eventType,items:[],symbols:new Set(),sectors:new Set()});
    const cluster=clusters.get(key)!;
    cluster.items.push(item);
    for(const sector of item.related_sectors??[])cluster.sectors.add(sector);
    const upper=raw.toUpperCase();
    for(const symbol of ['NVDA','TSM','AAPL','MSFT','META','AMZN','SOX','TSMC','2330'])if(upper.includes(symbol))cluster.symbols.add(symbol==='TSMC'?'TSM':symbol);
  }
  return Array.from(clusters.values()).slice(0,12).map((cluster)=>({
    topic:cluster.topic,
    headline_count:cluster.items.length,
    representative_headlines:cluster.items.slice(0,3).map((item)=>item.title),
    related_symbols:Array.from(cluster.symbols),
    related_sectors:Array.from(cluster.sectors),
    event_type:cluster.event_type,
    freshness:v10FreshnessLabel(cluster.items[0]?.published_at??cluster.items[0]?.created_at??null),
    source_items:cluster.items.slice(0,3).map((item)=>({id:item.id,title:item.title,source:item.source,url:item.url,published_at:item.published_at||item.created_at,taiwan_impact_summary:item.taiwan_impact_summary||''})),
  }));
}
function buildV10SectorContext(sectorData:SectorRotationRow[]):Record<string,unknown>[]{return (sectorData??[]).map((row)=>({sector:row.sector,score:row.rotation_score??null,direction:row.direction??null,reason:row.signal_label||row.summary||null,source_date:row.score_date??null,related_symbols:[...(row.leading_symbols??[]),...(row.lagging_symbols??[])]}));}
function buildResearchSectorContext(sectorData:SectorRotationRow[]):Record<string,unknown>[]{
  return buildV10SectorContext(sectorData).map((context,index)=>{
    const lineage=readSectorEvidenceLineage(sectorData[index]);
    if(!lineage)return context;
    return {...context,reason:sectorData[index].summary??context.reason,source:lineage.source,lineage};
  });
}
...
function buildV10EvidencePack(args:{todayDate:string;dates:{twCoreDate:string;usGlobalDate:string};tradingDayInfo:TradingDayInfo;marketData:MarketIndicator[];newsData:MarketNewsItem[];sectorData:SectorRotationRow[];previousReport:Record<string,unknown>|null;dataQuality:string;missingSources:string[];staleCoreSources:string[];unavailableSources:string[];confidenceResult:ReportConfidenceScore}):V10EvidencePack{const missing=new Set<string>(args.missingSources??[]);const stale=new Set<string>(args.staleCoreSources??[]);const available=new Set<string>();const get=(key:string,syms:string[])=>{const indicator=findIndicator(args.marketData,syms);if(!indicator){missing.add(`market_snapshot.${key}`);return null;}if(isCoreMarketDataStale(indicator,args.dates)){stale.add(`market_snapshot.${key}`);missing.add(`market_snapshot.${key}`);return null;}const value=normalizePremiumMarketEvidence(indicator);if(!value){missing.add(`market_snapshot.${key}`);return null;}available.add(`market_snapshot.${key}`);return value;};const marketSnapshot={report_date:args.todayDate,data_freshness:{tw_core_date:args.dates.twCoreDate,us_global_date:args.dates.usGlobalDate},taiwan_market_status:{is_trading_day:args.tradingDayInfo.is_trading_day,market_closed:args.tradingDayInfo.market_closed,holiday_name:args.tradingDayInfo.holiday_name??null},taiex:get('taiex',['TAIEX','TWII','^TWII']),txf:get('txf',['TXF','TX','MTX','TXF1']),'2330':get('2330',['2330','2330.TW']),tsm_adr:get('tsm_adr',['TSM']),spx:get('spx',['SPX','^GSPC']),nasdaq:get('nasdaq',['IXIC','NASDAQ','^IXIC']),sox:get('sox',['SOX','^SOX']),djia:get('djia',['DJIA','^DJI']),nvda:get('nvda',['NVDA']),aapl:get('aapl',['AAPL']),msft:get('msft',['MSFT']),meta:get('meta',['META']),amzn:get('amzn',['AMZN']),vix:get('vix',['VIX','^VIX']),dxy:get('dxy',['DXY','DX-Y.NYB']),us10y:get('us10y',['US10Y','TNX','^TNX']),crude_oil:get('crude_oil',['CL','WTI','CRUDE_OIL'])};if(args.marketData.length)available.add('market_data');else missing.add('market_data');if(args.newsData.length)available.add('market_news');else missing.add('market_news');if(args.sectorData.length)available.add('sector_rotation_scores');else missing.add('sector_rotation_scores');const previousValidation=buildV10PreviousValidation(args.previousReport);if(previousValidation)available.add('previous_report');else missing.add('previous_validation');for(const u of args.unavailableSources??[])missing.add(u);const warningFlags=[...(args.dataQuality==='degraded'?['data_quality_degraded']:[]),...(args.unavailableSources??[]).map((source)=>`unavailable:${source}`),...(stale.size?['stale_core_sources_present']:[])];return {market_snapshot:marketSnapshot,overnight_moves:buildV10OvernightMoves(args.marketData).filter((item)=>String(item.freshness_status??'').toLowerCase()!=='stale'),news_clusters:buildV10NewsClusters(args.newsData),sector_context:buildV10SectorContext(args.sectorData),previous_validation:previousValidation,data_quality:{available_sources:Array.from(available),missing_sources:Array.from(missing),stale_sources:Array.from(stale),confidence_base:args.confidenceResult.score,warning_flags:warningFlags}};}
function buildResearchEvidencePack(args:Parameters<typeof buildV10EvidencePack>[0]):V10EvidencePack{
  const pack=buildV10EvidencePack(args);
  if(!args.sectorData.some(row=>readSectorEvidenceLineage(row)))return pack;
  const quality=(pack.data_quality??{}) as Record<string,unknown>;
  const available=Array.isArray(quality.available_sources)?quality.available_sources.map(String):[];
  return {...pack,
    sector_context:buildResearchSectorContext(args.sectorData),
    data_quality:{...quality,available_sources:available.map(source=>source==='sector_rotation_scores'?'sector_rotation_derived_authoritative_close':source)},
  };
}
...
function normalizeV10MarketSnapshot(evidencePack:V10EvidencePack):Record<string,unknown>[]{const snapshot=(evidencePack.market_snapshot&&typeof evidencePack.market_snapshot==='object'?evidencePack.market_snapshot:{} ) as Record<string,unknown>;const defs=[['spx','SPX','index','US',82,60,'美股大盤風險偏好'],['nasdaq','NASDAQ','index','US',90,85,'科技股風險偏好'],['djia','DJIA','index','US',60,35,'美股傳統大型股風向'],['sox','SOX','index','US',98,96,'半導體族群對台股電子權值影響'],['tsm_adr','TSM','equity','US',96,98,'台積電 ADR 對 2330 與台股電子開盤情緒影響'],['nvda','NVDA','equity','US',94,92,'AI server 與半導體供應鏈風向'],['aapl','AAPL','equity','US',65,45,'大型科技與消費電子供應鏈情緒'],['msft','MSFT','equity','US',70,50,'AI 與雲端資本支出風向'],['meta','META','equity','US',60,45,'AI capex 與大型科技風險偏好'],['amzn','AMZN','equity','US',58,38,'雲端與大型科技風險偏好'],['vix','VIX','volatility','US',88,70,'市場風險溫度'],['dxy','DXY','fx','US',75,62,'美元強弱與外資資金壓力'],['us10y','US10Y','rate','US',78,68,'美債殖利率與估值壓力'],['crude_oil','CRUDE OIL','commodity','GLOBAL',55,35,'能源成本與通膨預期'],['taiex','TAIEX','index','TW',100,100,'台股現貨大盤方向'],['txf','TXF','futures','TW',95,95,'台指期與盤前/盤中風險方向'],['2330','2330','equity','TW',100,100,'台股最大權值與半導體核心驗證股']];return defs.map((def)=>{const [key,symbol,assetType,market,importance,taiwanRelevance,description]=def;const raw=snapshot[String(key)];if(!raw||typeof raw!=='object')return null;const r=raw as Record<string,unknown>;const changePercent=Number(r.change_percent);const freshness=String(r.freshness_status??'unknown').toLowerCase();if(!Number.isFinite(changePercent)||!r.updated_at||!['fresh','recent'].includes(freshness))return null;return {symbol,asset_type:assetType,market,value:r.value??null,direction:v10UpperDirection(r.direction),change_percent:changePercent,importance,taiwan_relevance:taiwanRelevance,description,raw_source:String(r.source??'market_data'),provider:r.provider??null,updated_at:r.updated_at,freshness,quality_status:'verified'};}).filter(Boolean) as Record<string,unknown>[];}
...
function buildEvidenceIndex(normalizedEvidence:Record<string,unknown>):V10EvidenceItem[]{
  const out:V10EvidenceItem[]=[];
  const push=(prefix:string,index:number,type:string,source:string,title:string,summary:string,importance:unknown,freshness:unknown,rawReference:string,extra:Partial<V10EvidenceItem>={})=>{out.push({evidence_id:`${prefix}${String(index+1).padStart(3,'0')}`,evidence_type:type,source,title,summary,importance:Number.isFinite(Number(importance))?Number(importance):0,freshness:String(freshness??'unknown'),raw_reference:rawReference,...extra});};
  const market=Array.isArray(normalizedEvidence.normalized_market_snapshot)?normalizedEvidence.normalized_market_snapshot:[];
  market.forEach((item,idx)=>{const m=(item&&typeof item==='object'?item:{} ) as Record<string,unknown>;const changePercent=Number(m.change_percent);const updatedAt=String(m.updated_at??'');if(!Number.isFinite(changePercent)||!updatedAt)return;const symbol=String(m.symbol??'');push('MD',idx,'market_data',String(m.raw_source??'market_data'),symbol,`${symbol} ${String(m.direction??'UNKNOWN')} ${changePercent.toFixed(2)}% ${String(m.description??'')}`,m.importance,m.freshness??'unknown',`market_data:${symbol}@${updatedAt}`,{published_at:updatedAt,quality_status:'verified'});});
  const news=Array.isArray(normalizedEvidence.normalized_news)?normalizedEvidence.normalized_news:[];
  news.forEach((item,idx)=>{const n=(item&&typeof item==='object'?item:{} ) as Record<string,unknown>;const url=String(n.url??'');const publishedAt=String(n.published_at??'');const source=String(n.source??'');if(!/^https?:\/\//i.test(url)||!publishedAt||!source)return;push('NEWS',idx,'market_news',source,String(n.topic??n.event_type??''),String(n.representative_summary??''),n.importance,n.freshness??'unknown',url,{published_at:publishedAt,url,quality_status:'verified'});});
  const sectors=Array.isArray(normalizedEvidence.sector_context)?normalizedEvidence.sector_context:[];
  sectors.forEach((item,idx)=>{const sec=(item&&typeof item==='object'?item:{} ) as Record<string,unknown>;const sourceDate=String(sec.source_date??'');if(!/^\d{4}-\d{2}-\d{2}$/.test(sourceDate))return;push('SEC',idx,'sector_rotation','sector_rotation_scores',String(sec.sector??''),String(sec.reason??sec.direction??''),sec.score??50,sourceDate?'previous_trading_day':'unknown',`sector_rotation_scores:${String(sec.sector??'')}@${sourceDate}`,{published_at:sourceDate||undefined,quality_status:'context'});});
  const prev=normalizedEvidence.previous_validation;
  if(prev&&typeof prev==='object'){const p=prev as Record<string,unknown>;push('VAL',0,'previous_validation','reports',String(p.previous_report_date??'previous_report'),String(p.previous_market_bias??''),70,'previous_report',`reports:${String(p.previous_report_date??'')}`,{published_at:String(p.previous_report_date??'')||undefined,quality_status:'context'});}
  return out;
}
function buildResearchEvidenceIndex(normalizedEvidence:Record<string,unknown>):V10EvidenceItem[]{
  const index=buildEvidenceIndex(normalizedEvidence);
  const sectors=Array.isArray(normalizedEvidence.sector_context)?normalizedEvidence.sector_context:[];
  return index.map((item)=>{
    if(item.evidence_type!=='sector_rotation')return item;
    const position=Number(item.evidence_id.slice(3))-1;
    const sector=(sectors[position]??{}) as Record<string,unknown>;
    const lineage=(sector.lineage??null) as SectorEvidenceLineage|null;
    if(lineage?.origin!=='DERIVED_EVIDENCE_RECONSTRUCTION')return item;
    const sourceDate=String(sector.source_date??'');
    return {...item,source:SECTOR_RECONSTRUCTION_SOURCE,
      raw_reference:`${SECTOR_RECONSTRUCTION_SOURCE}:batch:${lineage.batch_id}:${String(sector.sector??'')}@${sourceDate}`,
      published_at:undefined,data_as_of:sourceDate,source_fingerprint:lineage.batch_id};
  });
}
...
function v10EvidenceDirectionForCandidate(candidate:V10CandidateCard,item:V10EvidenceItem):V10EvidenceDirection{const base=v10ParseEvidenceMarketDirection(item);const title=item.title.toUpperCase();const industry=candidate.industry_code;const metadata=V10_CANDIDATE_METADATA[candidate.symbol];const defensive=Boolean(metadata?.tags.includes('DEFENSIVE'));const semanticTags=evidenceSemanticTags(item);if(defensive&&semanticTags.has('MARKET_RISK')){if(title==='VIX'&&base==='positive')return 'neutral';if(base==='negative')return 'neutral';if(base==='positive')return 'negative';return 'neutral';}const negativeForElectronics=['ELECTRONIC_BLUE_CHIP','AI_SERVER','PCB_CCL','COOLING','HIGH_SPEED_OPTICAL','MEMORY','IC_DESIGN','SEMI_EQUIPMENT_MATERIALS'];if(negativeForElectronics.includes(industry)&&['SOX','NVDA','TSM','NASDAQ','IXIC','2330'].some((key)=>title.includes(key)))return base;if(industry==='FINANCIAL'&&['DXY','US10Y'].some((key)=>title.includes(key))){if(base==='positive')return 'positive';if(base==='negative')return 'negative';return 'neutral';}if(industry==='CONSTRUCTION_ASSET'&&item.evidence_type==='market_news'&&/RATE HIKE|INFLATION.*HIGH|升息|通膨.*高/i.test(`${item.title} ${item.summary}`))return 'negative';if(industry==='SHIPPING'&&/(OIL|CRUDE|CL)/.test(title)){if(base==='negative')return 'positive';if(base==='positive')return 'negative';return 'neutral';}if(industry==='PETROCHEMICAL'&&/(OIL|CRUDE|CL)/.test(title))return base;if(item.evidence_type==='market_news'){const text=`${item.title} ${item.summary}`.toLowerCase();if(/risk|風險|tariff|關稅|war|跌|下修|miss|weak|利空/.test(text))return 'negative';if(/beat|strong|成長|上修|利多|policy|補助/.test(text))return 'positive';}return base==='negative'?'negative':base==='positive'?'positive':'neutral';}
...
function calculateV10BeneficiaryPhase1Record(
  candidate: V10CandidateCard,
  evidenceIndex: V10EvidenceItem[],
  primaryEvent: string,
): V10BeneficiaryPhase1Record {
  const refs = Array.isArray(candidate.related_evidence) ? candidate.related_evidence : [];
  const byId = v10EvidenceItemById(evidenceIndex);
  const evidenceSources = refs.map((ref) => byId.get(ref.evidence_id)).filter(Boolean)
    .map((item) => `${(item as V10EvidenceItem).title}｜${(item as V10EvidenceItem).raw_reference || 'evidence'}`)
    .filter(Boolean);
  const evidenceTypes = v10EvidenceTypesForRefs(refs, evidenceIndex);
  const directions = refs.map((ref) => byId.get(ref.evidence_id)).filter(Boolean)
    .map((item) => v10EvidenceDirectionForCandidate(candidate, item as V10EvidenceItem));
  const positive_evidence_count = directions.filter((direction) => direction === 'positive').length;
  const negative_evidence_count = directions.filter((direction) => direction === 'negative').length;
  const net_evidence_direction = v10NetEvidenceDirection(directions);
  const rawEventScore = v10ClampScore(refs.length > 0 ? v10AverageEvidenceWeight(refs) : 0);
  const directional_event_score = v10ClampScore(
    net_evidence_direction === 'positive'
      ? rawEventScore
      : net_evidence_direction === 'negative'
        ? rawEventScore * 0.25
        : rawEventScore * 0.6,
  );
  const sector_score = v10ClampScore(
    (candidate.trigger_tags.length > 0 ? 55 : 0)
      + (evidenceTypes.has('sector_rotation') ? 30 : 0)
      + (candidate.industry_code && primaryEvent.includes(candidate.industry_code) ? 15 : 0),
  );
  const evidence_score = v10ClampScore(Math.min(100, refs.length * 22 + evidenceTypes.size * 10));
  const market_score = v10ClampScore(candidate.liquidity_score);
  const repeat_penalty = v10ClampScore(candidate.repeat_penalty);
  const repeatPenaltyContribution = candidateRepeatPenaltyContribution(repeat_penalty);
  const directionPenalty = net_evidence_direction === 'negative' ? 22 : 0;
  const total_score = v10ClampScore(
    directional_event_score * 0.30
      + sector_score * 0.25
      + evidence_score * 0.20
      + market_score * 0.15
      - repeatPenaltyContribution
      - directionPenalty,
  );
  const confidence_level = v10ConfidenceLevelFromSignals(
    directional_event_score,
    sector_score,
    evidence_score,
    market_score,
    refs.length > 0,
  ...
  const eventLabel = v10CandidateEventLabel(candidate, refs, evidenceIndex);
  const benefit_chain = [
    eventLabel && eventLabel !== 'UNKNOWN' ? `事件：${eventLabel}` : '事件：尚未明確',
    candidate.trigger_tags.length
      ? `觸發標籤：${candidate.trigger_tags.filter((tag) =>
        v10IndustryRelevantTags(candidate.industry_code).has(tag)
        || refs.some((ref) => String(byId.get(ref.evidence_id)?.title ?? '').toUpperCase().includes(tag))
      ).join(', ') || candidate.trigger_tags.join(', ')}`
      : '觸發標籤：不足',
    `產業：${candidate.industry_code}`,
    `台灣供應鏈：${candidate.industry}`,
    `${candidate.symbol} ${candidate.name}`,
  ];
  const signalCount = [
    directional_event_score >= 50,
    sector_score >= 55,
    evidence_score >= 40,
    market_score >= 50,
  ...
  const risk_flags = [
    ...(net_evidence_direction === 'negative'
      ? ['negative_evidence_blocks_high_beneficiary', 'exclude_from_top3_strong_beneficiary']
      : []),
    ...(negative_evidence_count > 0 ? [`negative_evidence_count=${negative_evidence_count}`] : []),
  ];
  const scoring_reasons = [
    `event_score=${rawEventScore}`,
    `directional_event_score=${directional_event_score}`,
    `sector_score=${sector_score}`,
    `evidence_score=${evidence_score}`,
    `market_score=${market_score}`,
    `repeat_penalty=${repeat_penalty}`,
    `repeat_penalty_contribution=${repeatPenaltyContribution}`,
    `positive_evidence_count=${positive_evidence_count}`,
    `negative_evidence_count=${negative_evidence_count}`,
    `net_evidence_direction=${net_evidence_direction}`,
    `confidence_signals_passed=${signalCount}`,
    refs.length > 0 ? `evidence_refs=${refs.map((ref) => ref.evidence_id).join(',')}` : 'evidence_refs=none',
    ...(risk_flags.length ? [`risk_flags=${risk_flags.join(',')}`] : []),
  ];
  return {
    symbol: candidate.symbol,
    name: candidate.name,
    industry_code: candidate.industry_code,
    industry_name: candidate.sector,
    company_role: candidate.industry,
    trigger_event: eventLabel,
    ...
    benefit_chain,
    scoring_reasons,
    data_quality_status: refs.length > 0 ? 'sufficient' : 'insufficient',
  };
}
...
function v11ObservationSourceRefs(refs:V10EvidenceReference[],evidenceIndex:V10EvidenceItem[]):string[]{const byId=v10EvidenceItemById(evidenceIndex);return Array.from(new Set(refs.map((ref)=>{const item=byId.get(ref.evidence_id);if(!item)return'';const detail=[item.title,item.summary].filter(Boolean).join('：');return `${ref.evidence_id}｜${item.source}.${item.raw_reference||item.title}｜${detail}`;}).filter(Boolean))).slice(0,4);}
...
function outputV11ObservationRecord(record:V11ObservationRecord):Record<string,unknown>{return {symbol:record.symbol,name:record.name,industry_code:record.industry_code,industry_name:record.industry_name,rank:record.rank,total_score:record.observation_score,observation_score:record.observation_score,event_relevance:record.event_relevance,confirmation_pending:record.confirmation_pending,market_attention:record.market_attention,industry_representative:record.industry_representative,repeat_penalty:record.repeat_penalty,confidence_level:record.confidence_level,net_evidence_direction:record.net_evidence_direction,positive_evidence_count:record.positive_evidence_count,negative_evidence_count:record.negative_evidence_count,risk_flags:record.risk_flags,observation_reason:record.observation_reason,confirmation_pending_reason:record.confirmation_pending_reason,stop_observing_condition:record.stop_observing_condition,observation_chain:record.observation_chain,benefit_chain:record.observation_chain,source_refs:record.source_refs,data_basis:record.data_basis,scoring_reasons:[`今天為什麼觀察？${record.observation_reason}`,`還缺什麼？${record.confirmation_pending_reason}`,`不用再觀察的條件：${record.stop_observing_condition}`],data_quality_status:record.data_quality_status};}

function v11CurrentObservationEvidencePriority(item:V10EvidenceItem):number{
  if(v11RoleIsFreshNews(item))return 0;
  if(item.evidence_type==='market_data'&&/^(TAIEX|TXF|2330)$/i.test(String(item.title)))return 1;
  if(v11RoleIsExternalEvidence(item))return 2;
  if(item.evidence_type==='market_data')return 3;
  return 9;
}
function v11IsCurrentObservationEvidence(item:V10EvidenceItem):boolean{
  if(item.evidence_type==='previous_validation')return false;
  const freshness=String(item.freshness??'').toLowerCase();
  if(!['fresh','recent'].includes(freshness))return false;
  return item.evidence_type==='market_news'||item.evidence_type==='market_data';
}
function v11FormatObservationEvidence(item:V10EvidenceItem):string{
  return `${item.evidence_id}｜${item.source}.${item.raw_reference||item.title||item.evidence_id}｜${item.summary||item.title||item.evidence_id}`.trim();
}
function applyV11CurrentObservationEvidence(records:Record<string,unknown>[],evidenceIndex:V10EvidenceItem[]):Record<string,unknown>[] {
  const currentRefs=Array.from(new Set([...evidenceIndex]
    .filter(v11IsCurrentObservationEvidence)
    .sort((a,b)=>v11CurrentObservationEvidencePriority(a)-v11CurrentObservationEvidencePriority(b)||(Number(b.importance)||0)-(Number(a.importance)||0))
    .map(v11FormatObservationEvidence)
    .filter(Boolean))).slice(0,4);
  return records.map((record)=>{
    const existing=Array.isArray(record.source_refs)?record.source_refs.map(String):[];
    const existingCurrent=existing.filter((ref)=>!/^VAL\d*｜/i.test(ref)&&!/previous_validation|reports:\d{4}-\d{2}-\d{2}/i.test(ref));
    const sourceRefs=Array.from(new Set([...currentRefs,...existingCurrent])).slice(0,4);
    const riskFlags=Array.isArray(record.risk_flags)?record.risk_flags.map(String):[];
    if(sourceRefs.length===0)riskFlags.push('current_observation_evidence_missing');
    return {
      ...record,
      source_refs:sourceRefs,
      data_basis:sourceRefs.join('；'),
      data_quality_status:sourceRefs.length>0?record.data_quality_status:'insufficient',
...
function v11RoleEvidenceTitle(evidenceIndex:V10EvidenceItem[],matcher?:(item:V10EvidenceItem)=>boolean):string{const item=evidenceIndex.find((entry)=>matcher?matcher(entry):true);if(!item)return '外部市場變數';return item.evidence_type==='market_news'?(item.summary||item.title||item.evidence_id):(item.title||item.summary||item.evidence_id);}
function v11RolePickDifferent(records:Record<string,unknown>[],baseIndustry:string):Record<string,unknown>|undefined{return records.find((record)=>{const industry=v11RoleText(record.industry_code);return Boolean(industry)&&industry!==baseIndustry;});}
...
function v11RoleDecisionConfidence(source:Record<string,unknown>|undefined,evidenceIndex:V10EvidenceItem[],role:V11ObservationRole):number{const data=String(source?.data_quality_status??'');const direction=String(source?.net_evidence_direction??'neutral');const positive=Number(source?.positive_evidence_count??0)||0;const negative=Number(source?.negative_evidence_count??0)||0;const score=Number(source?.total_score??source?.observation_score??50);const dataScore=data==='sufficient'?26:data==='partial'?18:10;const marketScore=direction==='positive'?24:direction==='neutral'?16:direction==='negative'?10:14;const evidenceScore=Math.min(24,Math.max(8,evidenceIndex.length*3+positive*5-negative*4));const roleScore=role==='CONFIRMATION'||role==='RISK'?16:role==='MAIN_THESIS'?18:14;const raw=v10ClampScore(dataScore+marketScore+evidenceScore+roleScore+(Number.isFinite(score)?score*0.12:0));const dataCap=data==='sufficient'?85:data==='partial'?65:50;return Math.min(dataCap,raw);}
function v11RoleIsFreshNews(item:V10EvidenceItem):boolean{return item.evidence_type==='market_news'&&['fresh','recent'].includes(String(item.freshness).toLowerCase());}
function v11RoleIsExternalEvidence(item:V10EvidenceItem):boolean{if(item.evidence_type==='market_news'||item.evidence_type==='macro')return true;if(item.evidence_type!=='market_data')return false;const text=[item.title,item.summary,item.raw_reference].join(' ').toUpperCase();if(/\b(TAIEX|TWII|TXF|MTX|2330(?:\.TW)?)\b/.test(text))return false;return /\b(SOX|SPX|GSPC|NASDAQ|IXIC|DJIA|DJI|NVDA|TSM|AAPL|MSFT|META|AMZN|VIX|DXY|US10Y|TNX|WTI|CL|CRUDE(?:_OIL)?)\b/.test(text);}
function v11RoleExternalPriority(evidenceIndex:V10EvidenceItem[]):{title:string;importance:number}[]{return evidenceIndex.filter(v11RoleIsExternalEvidence).slice(0,8).map((item)=>({title:item.evidence_type==='market_news'?(item.summary||item.title||item.evidence_id):(item.title||item.summary||item.evidence_id),importance:Math.max(1,Math.min(5,Math.round((Number(item.importance??50)||50)/20)))})).filter((item)=>item.title).slice(0,3);}
...
function buildV11ObservationNarrativeRoles(input:{legacyObservation:Record<string,unknown>[];riskWatchlist:Record<string,unknown>[];strongWatchlist:Record<string,unknown>[];topCandidates:V10BeneficiaryPhase1Record[];candidateUniverse:Record<string,unknown>;evidenceIndex:V10EvidenceItem[];dataQualityStatus:string;warning:string|null;log?:(msg:string)=>void}):Record<string,unknown>[]|null{
  try{
    const legacy=Array.isArray(input.legacyObservation)?input.legacyObservation:[];
    const risks=Array.isArray(input.riskWatchlist)?input.riskWatchlist:[];
    const strong=Array.isArray(input.strongWatchlist)?input.strongWatchlist:[];
    const top=(Array.isArray(input.topCandidates)?input.topCandidates:[]).map((r)=>({...r,total_score:r.total_score})) as unknown as Record<string,unknown>[];
    const freshNews=input.evidenceIndex.find(v11RoleIsFreshNews);
    const externalEvidence=input.evidenceIndex.find(v11RoleIsExternalEvidence);
    const main=strong[0];
    const mainIndustry=v11RoleIndustry(main);
    const capital=v11RolePickDifferent([...strong,...legacy,...top],mainIndustry.code);
    if(input.dataQualityStatus!=='sufficient'||!main||!freshNews||!externalEvidence||!capital){
      input.log?.('V11_OBSERVATION_ROLE_BUILDER_SKIPPED '+JSON.stringify({data_quality_status:input.dataQualityStatus,strong_count:strong.length,has_fresh_news:Boolean(freshNews),has_external_evidence:Boolean(externalEvidence),has_distinct_capital_candidate:Boolean(capital),warning:input.warning}));
      return null;
    }
    const confirm=legacy.find((r)=>v11RoleText(r.industry_code)===mainIndustry.code)||main;
    const risk=risks[0]||top.find((r)=>v11RoleText(r.net_evidence_direction)==='negative')||main;
    const externalTitle=v11RoleEvidenceTitle([freshNews]);
    const externalSymbol=v11RoleEvidenceTitle([externalEvidence]);
    const mainSymbols=v11RoleSymbols(main);
    const mainNames=v11RoleNames(main);
    const capitalIndustry=v11RoleIndustry(capital);
    const riskIndustry=v11RoleIndustry(risk);
    const mainTarget=[...mainSymbols,...mainNames].filter(Boolean).slice(0,2).join(' / ')||'待確認代表股';
    const capitalTarget=[...v11RoleSymbols(capital),...v11RoleNames(capital)].filter(Boolean).slice(0,2).join(' / ')||capitalIndustry.name;
    const riskTarget=[...v11RoleSymbols(risk),...v11RoleNames(risk)].filter(Boolean).slice(0,2).join(' / ')||riskIndustry.name;
    const confirmationChecklist=[`確認 ${mainTarget} 是否轉強`,`確認 TAIEX 與 TXF 是否同向`,`確認 ${mainIndustry.name} 是否有族群擴散`];
    const riskChecklist=[`觀察 ${riskTarget} 是否轉弱`,'觀察 TAIEX 是否失去支撐',`觀察 ${mainIndustry.name} 是否量縮`];
    const rotationPath=Array.from(new Set([mainIndustry.name,capitalIndustry.name,'防禦族群','明日主線候選'].filter(Boolean))).slice(0,5);
    const externalPriority=v11RoleExternalPriority(input.evidenceIndex);
    const records:Record<string,unknown>[]=[
      v11RoleBaseRecord('MAIN_THESIS',1,main,{narrative:`${externalTitle} 先改變海外風險偏好，再透過 ${mainIndustry.name} 傳導到台股，受惠方向要看 ${mainTarget} 是否承接資金。`,validation_point:`先看 ${mainTarget} 是否與 TAIEX、TXF 同向。`,stop_condition:`若 ${mainIndustry.name} 代表股無法同步或量能不足，今日主線降級。`,observation_chain:[externalTitle,'海外風險偏好',mainIndustry.name,mainTarget].filter(Boolean),decision_confidence:v11RoleDecisionConfidence(main,input.evidenceIndex,'MAIN_THESIS')}),
      v11RoleBaseRecord('CONFIRMATION',2,confirm,{narrative:'今天不是只看單一標的，劇本成立需要權值股、指數結構與主線族群同時配合。',validation_point:`09:30 / 10:30 確認 2330、TAIEX、TXF 與 ${mainIndustry.name} 是否同向。`,stop_condition:'若只有單點上漲、族群沒有擴散，今天不把它升級成強主線。',representative_symbols:Array.from(new Set([...mainSymbols,'2330'])).slice(0,2),representative_names:Array.from(new Set([...mainNames,'台積電'])).slice(0,2),industry_code:mainIndustry.code,industry_name:mainIndustry.name,confirmation_checklist:confirmationChecklist,observation_chain:['權值股','指數結構',mainIndustry.name],decision_confidence:v11RoleDecisionConfidence(confirm,input.evidenceIndex,'CONFIRMATION')}),
      v11RoleBaseRecord('RISK',3,risk,{narrative:`今天最大的風險是 ${riskIndustry.name} 的反向訊號把早上的劇本打斷，而不是名單裡少一檔股票。`,validation_point:`觀察 ${riskIndustry.name} 代表股是否轉弱，並確認 TAIEX 是否失去支撐。`,stop_condition:v11RoleText(risk?.stop_observing_condition,`若 ${riskIndustry.name} 轉弱且權值股不同步，今日劇本視為失效。`),risk_checklist:riskChecklist,observation_chain:['反向訊號',riskIndustry.name,'劇本失效條件'],decision_confidence:v11RoleDecisionConfidence(risk,input.evidenceIndex,'RISK')}),
    ...
    const observationSourceRefs=[...input.evidenceIndex]
      .sort((a,b)=>{
        const rank=(item:V10EvidenceItem):number=>v11RoleIsFreshNews(item)?0:v11RoleIsExternalEvidence(item)?1:item.evidence_type==='market_data'?2:item.evidence_type==='previous_validation'?9:5;
        return rank(a)-rank(b)||(Number(b.importance)||0)-(Number(a.importance)||0);
      })
      .slice(0,4)
      .map((item)=>`${item.evidence_id}｜${item.source}.${item.raw_reference||item.title||item.evidence_id}｜${item.summary||item.title||item.evidence_id}`.trim())
      .filter(Boolean);
...
function buildV10BeneficiaryPhase1(candidateUniverse:Record<string,unknown>,evidenceIndex:V10EvidenceItem[]):V10BeneficiaryPhase1Output{
  const candidates=Array.isArray(candidateUniverse.candidates)?candidateUniverse.candidates as V10CandidateCard[]:[];
  const eligible=candidates.filter((candidate)=>candidate.eligibility!==false);
  const candidateBySymbol=new Map(eligible.map((candidate)=>[candidate.symbol,candidate]));
  const evidenceById=v10EvidenceItemById(evidenceIndex);
  const primaryEvent=String(candidateUniverse.primary_event??'UNKNOWN');
  const scored=eligible.map((candidate)=>calculateV10BeneficiaryPhase1Record(candidate,evidenceIndex,primaryEvent))
    .sort((a,b)=>b.total_score-a.total_score||b.evidence_score-a.evidence_score||a.repeat_penalty-b.repeat_penalty)
    .map((record,index)=>({...record,rank:index+1}));
  const toOutputRecord=(record:V10BeneficiaryPhase1Record,recommendationState:'confirmed'|'conditional'='confirmed')=>({
    symbol:record.symbol,name:record.name,industry_code:record.industry_code,industry_name:record.industry_name,
    rank:record.rank,total_score:record.total_score,confidence_level:record.confidence_level,
    directional_event_score:record.directional_event_score,positive_evidence_count:record.positive_evidence_count,
    negative_evidence_count:record.negative_evidence_count,net_evidence_direction:record.net_evidence_direction,
    risk_flags:record.risk_flags,trigger_event:record.trigger_event,data_basis:record.evidence_sources.join('；'),
    evidence_source:record.evidence_sources[0]||'',transmission_logic:record.benefit_chain.join(' → '),
    taiwan_supply_chain_link:`${record.name} 是台灣 ${record.industry_name} 中的 ${record.company_role} 代表公司，事件必須先傳導到同族群後才成立。`,
    intraday_validation:`09:30 確認 ${record.symbol} ${record.name} 相對加權指數強勢，且 ${record.industry_name} 至少兩檔代表股同步。`,
    entry_condition:`${record.symbol} 相對加權指數轉強，且 ${record.industry_name} 至少兩檔代表股同步放量後，條件才成立。`,
    invalidation_condition:`若 ${record.symbol} ${record.name} 弱於加權指數，或 ${record.industry_name} 沒有量價同步，受惠假設失效。`,
    recommendation_state:recommendationState,direction:recommendationState==='confirmed'?'證據確認':'條件式推薦',
    not_buy_signal:recommendationState==='conditional',benefit_chain:record.benefit_chain,
    scoring_reasons:[...record.scoring_reasons,`recommendation_state=${recommendationState}`],data_quality_status:record.data_quality_status,
  });
  const currentDecisionEvidenceCount=(record:V10BeneficiaryPhase1Record):number=>{
    const candidate=candidateBySymbol.get(record.symbol);
    if(!candidate)return 0;
    return candidate.related_evidence.filter((ref)=>evidenceById.get(ref.evidence_id)?.evidence_type!=='previous_validation').length;
  };
  const riskRecords=scored.filter((record)=>record.net_evidence_direction==='negative');
  const positiveBacked=scored.filter((record)=>record.data_quality_status==='sufficient'&&record.net_evidence_direction==='positive'&&record.positive_evidence_count>=1&&record.positive_evidence_count>record.negative_evidence_count&&['medium','high'].includes(record.confidence_level)&&record.total_score>=45&&currentDecisionEvidenceCount(record)>=1);
  const conditionalBacked=scored.filter((record)=>record.data_quality_status==='sufficient'&&record.net_evidence_direction==='neutral'&&['medium','high'].includes(record.confidence_level)&&record.total_score>=40&&currentDecisionEvidenceCount(record)>=2);
  const noCandidates=scored.length===0;
  const insufficient=noCandidates;
  const confirmedTop=buildV10DiverseTopCandidates(positiveBacked,5).map((record,index)=>({...record,rank:index+1}));
  const conditionalTop=buildV10DiverseTopCandidates(conditionalBacked,3).map((record,index)=>({...record,rank:index+1}));
  const selected=confirmedTop.length>0?confirmedTop:conditionalTop;
  const recommendationState:'confirmed'|'conditional'=confirmedTop.length>0?'confirmed':'conditional';
  const today=insufficient||selected.length===0?[]:selected.map((record)=>toOutputRecord(record,recommendationState));
  const noRecommendation=!insufficient&&today.length===0;
  const legacyObservationWatchlist=buildV11ObservationWatchlist(eligible,scored,riskRecords,selected,evidenceIndex,10).map(outputV11ObservationRecord);
  const riskWatchlist=riskRecords.slice(0,10).map((record)=>toOutputRecord(record));
  const status=insufficient?'insufficient':noRecommendation?'insufficient_positive_evidence':(recommendationState==='conditional'||today.length<5?'partial':'sufficient');
  const warning=insufficient?'今日事件、產業或行情驗證資料不足，未輸出 V10 推薦股':noRecommendation?'今日沒有足夠可追溯證據建立推薦股':recommendationState==='conditional'?'今日沒有直接多方確認，提供具來源、進場條件與失效條件的條件式推薦；條件未成立前不可視為進場訊號':null;
  const roleObservationWatchlist=buildV11ObservationNarrativeRoles({legacyObservation:legacyObservationWatchlist,riskWatchlist,strongWatchlist:today,topCandidates:scored.slice(0,10),candidateUniverse,evidenceIndex,dataQualityStatus:status,warning,log:(msg)=>console.log(msg)});
  const observationWatchlist=applyV11CurrentObservationEvidence(roleObservationWatchlist??legacyObservationWatchlist,evidenceIndex);
  const validation=validateV10BeneficiaryPhase1(scored.slice(0,10),candidateUniverse);
  return {enabled:false,candidate_count:scored.length,data_quality_status:status,warning,top_10_candidates:scored.slice(0,10),today_beneficiary_stocks_v10:today,observation_watchlist:observationWatchlist,risk_watchlist:riskWatchlist,validation};
}

function v10NarrativeText(value:unknown,maxLength=52):string{return String(value??'').replace(/\s+/g,' ').trim().replace(/[。；;]+$/,'').slice(0,maxLength);}
function v10NarrativeSourceDetail(sourceRefs:string[]):string{
  for(const ref of sourceRefs){
    if(/^VAL\d*｜/i.test(ref)||/previous_validation|reports:\d{4}-\d{2}-\d{2}/i.test(ref))continue;
    const parts=ref.split('｜');
    const detail=v10NarrativeText(parts[2]||parts[1]||parts[0],28);
    if(detail&&!/^\d{4}-\d{2}-\d{2}$/.test(detail)&&!/^https?:\/\//i.test(detail))return detail;
  }
  return '今日隔夜消息與市場訊號';
}
function applyV10EvidenceBackedNarrative(ai:Record<string,unknown>,phase:V10BeneficiaryPhase1Output):Record<string,unknown>{
  for(const row of phase.today_beneficiary_stocks_v10){if(row&&typeof row==='object')Object.assign(row,enrichV10PremiumStockRecord(row));}
  for(const row of phase.risk_watchlist){if(row&&typeof row==='object')Object.assign(row,enrichV10PremiumStockRecord(row));}
  const recommendations=Array.isArray(phase.today_beneficiary_stocks_v10)?phase.today_beneficiary_stocks_v10:[];
  const observations=Array.isArray(phase.observation_watchlist)?phase.observation_watchlist:[];
  const recommendation=recommendations[0]&&typeof recommendations[0]==='object'?recommendations[0] as Record<string,unknown>:null;
  const observation=observations[0]&&typeof observations[0]==='object'?observations[0] as Record<string,unknown>:null;
  let sentence='';let subscriberSentence='';let logicSource:string[]=[];let decisionMode='';
  if(recommendation){
  ...
  const v8=ai.v8_daily_sentence&&typeof ai.v8_daily_sentence==='object'&&!Array.isArray(ai.v8_daily_sentence)?{...(ai.v8_daily_sentence as Record<string,unknown>)}:{};
  const free=ai.free_summary&&typeof ai.free_summary==='object'&&!Array.isArray(ai.free_summary)?{...(ai.free_summary as Record<string,unknown>)}:{};
  const line=ai.line_push_copy&&typeof ai.line_push_copy==='object'&&!Array.isArray(ai.line_push_copy)?{...(ai.line_push_copy as Record<string,unknown>)}:{};
  const note=ai.member_research_note_v2&&typeof ai.member_research_note_v2==='object'&&!Array.isArray(ai.member_research_note_v2)?{...(ai.member_research_note_v2 as Record<string,unknown>)}:{};
  return {...ai,today_quote:sentence,v8_daily_sentence:{...v8,status:'ready',sentence,logic_source:logicSource,decision_mode:decisionMode},free_summary:{...free,one_sentence:sentence},line_push_copy:{...line,one_sentence:sentence},member_research_note_v2:{...note,subscriber_value_sentence:subscriberSentence,today_core_thesis:sentence}};
}
...
function applyBiasGuardrails(md:MarketIndicator[],baseScore:number,dates?:MarketFreshnessDates):BiasGuardrailResult{
  ...
  const usable=function(m:MarketIndicator|null,label:string):MarketIndicator|null{if(!m)return null;if(isCoreMarketDataStale(m,dates)){if(label==='TXF'){unavailableSignals.push('TXF 資料暫無授權來源或近月合約對應，今日不納入方向扣分');return null;}staleSignals.push(label+' 資料過期（'+(m.updatedAt||'無時間')+'）');return null;}return m};
...
function buildGuardedLinePushCopy(todayDate:string,marketBias:string,confidenceScore:number,md:MarketIndicator[],guard:BiasGuardrailResult,dates?:MarketFreshnessDates):Record<string,unknown>{
  ...
  if(tsm&&!isCoreMarketDataStale(tsm,dates)&&tsm.changePercent<=-2)one='TSM ADR '+fmtSignedPct(tsm.changePercent)+' 壓低電子權值，今日先看 2330 是否止穩。';
  else if(sox&&!isCoreMarketDataStale(sox,dates)&&sox.changePercent<=-3)one='SOX '+fmtSignedPct(sox.changePercent)+' 拖累半導體情緒，今日不宜用偏多劇本硬追。';
  else if(nvda&&!isCoreMarketDataStale(nvda,dates)&&nvda.changePercent<=-2)one='NVDA '+fmtSignedPct(nvda.changePercent)+' 轉弱，AI 供應鏈先看抗跌不看追價。';
  else if(nasdaq&&!isCoreMarketDataStale(nasdaq,dates)&&nasdaq.changePercent<=-1.5)one='NASDAQ '+fmtSignedPct(nasdaq.changePercent)+' 走弱，台股成長股今日先降速觀察。';
  ...
  const risk=vix&&!isCoreMarketDataStale(vix,dates)&&vix.value>=22?'VIX '+vix.value.toFixed(1)+'，市場風險偏高':mainRisk;
...
function applyFinalBiasGuardrails(ai:Record<string,unknown>,todayDate:string,md:MarketIndicator[],dScore:MarketDataScore,confidenceResult:ReportConfidenceScore,dates:MarketFreshnessDates):Record<string,unknown>{
  const originalScore=Number(dScore.details.bias_guardrail_original_score);
  const guard=applyBiasGuardrails(md,Number.isFinite(originalScore)?originalScore:dScore.baseScore,dates);const guardedBias=classifyMarketBias(guard.adjustedScore);const out={...ai};
  ...
  if(guard.shouldDowngrade){
    const copy=buildGuardedLinePushCopy(todayDate,guardedBias,guardedConfidence,md,guard,dates);out.line_push_copy=copy;
...
function stockResearchProfile(symbol:string,sector:string,industryCode:string=''):Record<string,string>{
  ...
  if(['2368','3037','3044','5469','2383','6274','6213'].includes(symbol))return{first_order_impact:'AI 伺服器、高速運算或半導體需求先改變高階 PCB、CCL 與載板訂單預期。',second_order_impact:'只有伺服器板材規格升級與相關族群同步表態，題材才會傳到材料與板端。',taiwan_supply_chain_link:'該股位於台灣 PCB、CCL 或 ABF 載板環節，是伺服器與半導體訊號往零組件端傳導的驗證點。',why_this_stock:'它反映的不是泛 AI 題材，而是高階板材、載板或銅箔基板需求是否真正增加。',validation_signal:'10:00 前確認 PCB / CCL 族群至少兩檔同步強於 TAIEX，且成交量不是單一個股事件。'};
  if(['3017','3324','3653'].includes(symbol))return{first_order_impact:'AI 伺服器功耗與機櫃密度上升，先提高散熱模組、均熱片或液冷需求預期。',second_order_impact:'GPU 與伺服器出貨訊號必須延續，市場才會把需求傳到散熱零組件。',taiwan_supply_chain_link:'該股位於台灣伺服器散熱鏈，負責把高功耗運算需求轉成熱管理零組件與模組需求。',why_this_stock:'它用來驗證 AI 伺服器訂單是否真的外溢到散熱端，而不是只停留在 GPU 題材。',validation_signal:'09:30 至 10:30 確認散熱族群同步、成交量放大，並且 AI 伺服器 ODM 沒有轉弱。'};
  if(['3081','4979','4908','3363','3450','2345','4906'].includes(symbol))return{first_order_impact:'資料中心頻寬升級先提高高速光模組、雷射元件與交換器需求預期。',second_order_impact:'雲端資本支出與網路升級必須延續，資金才會從伺服器主線擴散到高速傳輸。',taiwan_supply_chain_link:'該股位於台灣光通訊或高速網路設備鏈，連結資料中心 800G / 1.6T 升級需求。',why_this_stock:'它是驗證 AI 基礎設施是否從運算擴散到傳輸瓶頸的代表標的。',validation_signal:'10:00 前確認光通訊或網通代表股至少兩檔同步強於 TAIEX，且不是開高走低。'};
  if(['2408','2344','8299','3260'].includes(symbol))return{first_order_impact:'Micron、記憶體報價與 AI 記憶體需求先改變 DRAM、NAND 或控制晶片景氣預期。',second_order_impact:'報價、庫存與需求訊號必須一致，才會傳到台灣記憶體製造、模組與控制晶片。',taiwan_supply_chain_link:'該股位於台灣 DRAM、Flash、記憶體模組或控制晶片環節，對報價與庫存循環敏感。',why_this_stock:'它用來驗證全球記憶體訊號是否真的轉化為台灣記憶體族群資金流。',validation_signal:'09:30 至 10:30 確認記憶體族群同步、成交量擴大，且上游報價訊號沒有反轉。'};
  ...
  if(['3131','3167','3583','6187','4763','1560','3711','6239','8046'].includes(symbol))return{first_order_impact:'晶圓廠資本支出、先進製程或先進封裝需求先改變設備、材料與封測訂單預期。',second_order_impact:'只有資本支出與產能利用率訊號延續，需求才會傳到台灣設備、材料、耗材與封裝環節。',taiwan_supply_chain_link:'該股位於台灣半導體設備、材料、耗材或封裝測試鏈，承接晶圓製造擴產與製程升級需求。',why_this_stock:'它用來驗證半導體景氣是否從權值股進一步傳到資本支出與後段製程。',validation_signal:'10:00 前確認設備材料或封測族群至少兩檔同步，且 2330 與 SOX 訊號沒有反向。'};
  if(['2881','2882','2886','2891','5880','2884'].includes(symbol))return{first_order_impact:'VIX、DXY 與美債訊號會先改變外資風險胃納。',second_order_impact:'風險升高時，金融權值可能承接防禦配置或成為外資調節觀察點。',taiwan_supply_chain_link:'金融股不是供應鏈受惠，而是資金流與防禦配置的代表。',why_this_stock:'它是金融權值代表，能檢查市場是否從成長股轉向防禦配置。',validation_signal:'09:30 觀察金融指數是否相對 TAIEX 抗跌，以及外資金融權值買賣方向。'};
  ...
  if(['1301','1303','1326','6505'].includes(symbol))return{first_order_impact:'CL / 原油變動先影響塑化原料成本與利差預期。',second_order_impact:'若油價變動具有延續性，塑化族群才會出現基本面重估。',taiwan_supply_chain_link:'該股屬塑化上游，對油價與利差變化敏感。',why_this_stock:'它是塑化鏈代表，能驗證原物料訊號是否真正傳到台股循環股。',validation_signal:'盤中觀察塑化族群是否同步、油價期貨是否延續同向，以及成交量是否放大。'};
  if(['2603','2609','2615','2606','2617','2610'].includes(symbol))return{first_order_impact:'油價與景氣風險先影響航運成本與運價預期。',second_order_impact:'航運股需要運價或油價訊號延續，才有族群性反應。',taiwan_supply_chain_link:'該股代表航運成本與全球貿易循環敏感度。',why_this_stock:'它是航運族群代表，可用來驗證原物料與景氣訊號是否轉化為循環股資金流。',validation_signal:'盤中觀察航運族群同步性、運價相關新聞與油價方向是否一致。'};
  if(['1513','1504','1605','1609','1611','1519','6443'].includes(symbol))return{first_order_impact:'電網建設、資料中心用電或能源政策先改變重電、電力設備與線纜訂單預期。',second_order_impact:'政策預算與在手訂單必須延續，資金才會從題材傳到設備交付與營收能見度。',taiwan_supply_chain_link:'該股位於台灣重電、電網、電力設備或線纜供應鏈，承接輸配電與用電升級需求。',why_this_stock:'它用來驗證電力需求是否落到具體設備與工程訂單，而不是只剩政策口號。',validation_signal:'10:00 前確認重電與電網族群同步、成交量放大，且政策或訂單來源仍有效。'};
  if(['2201','2204','1536','3665','2207'].includes(symbol))return{first_order_impact:'車市需求、電動車產量、匯率或海外供應鏈事件先改變汽車與零組件訂單預期。',second_order_impact:'整車銷售或客戶拉貨必須改善，訊號才會傳到台灣零組件、線束與通路。',taiwan_supply_chain_link:'該股位於台灣汽車整車、零組件、線束或通路環節，對車市與海外訂單變化敏感。',why_this_stock:'它用來驗證海外車市訊號是否真的落到台灣汽車供應鏈出貨，而不是泛電動車題材。',validation_signal:'盤中確認汽車零組件族群同步、代表股量能放大，且匯率或車市來源沒有反轉。'};
  if(industryCode==='CONSTRUCTION_ASSET')return{first_order_impact:'利率、房市政策與資產價格先改變營建推案、土地資產與融資成本預期。',second_order_impact:'政策與成交量必須同步改善，訊號才會傳到營建與建材公司的獲利能見度。',taiwan_supply_chain_link:'該股位於台灣營建、資產或建材鏈，對本地利率、房市政策與推案節奏敏感。',why_this_stock:'它用來驗證政策或利率變化是否真的轉化為台灣房市成交與營建資金流。',validation_signal:'盤中確認營建資產族群同步、成交量放大，且利率或政策來源仍支持原方向。'};
  if(industryCode==='AIRLINE_TOURISM')return{first_order_impact:'油價、匯率與旅遊需求先改變航空成本、票價與觀光消費預期。',second_order_impact:'訂位、載客或消費訊號必須延續，才會傳到航空、旅行社、飯店與餐飲。',taiwan_supply_chain_link:'該股位於台灣航空或觀光消費鏈，對燃油成本、匯率與旅遊需求敏感。',why_this_stock:'它用來驗證海外旅遊與成本訊號是否真的落到台灣觀光族群營運。',validation_signal:'盤中確認航空觀光族群同步，並對照油價與匯率是否支持原假設。'};
  if(industryCode==='RETAIL_CONSUMER'||industryCode==='FOOD')return{first_order_impact:'消費信心、匯率與原物料價格先改變內需銷售、進口成本與毛利預期。',second_order_impact:'同店銷售或通路需求必須改善，訊號才會傳到零售、食品與民生消費股。',taiwan_supply_chain_link:'該股位於台灣零售、通路或食品消費鏈，反映內需與成本變化。',why_this_stock:'它用來驗證總體消費訊號是否真正轉化為台灣內需營收與資金防禦配置。',validation_signal:'盤中確認內需消費族群相對 TAIEX 抗跌或同步轉強，且不是單一公司事件。'};
  if(industryCode==='STEEL'||industryCode==='CEMENT')return{first_order_impact:'原物料價格、關稅與基礎建設需求先改變報價、成本與庫存預期。',second_order_impact:'報價與需求必須同步，訊號才會傳到台灣鋼鐵、水泥或建材公司的利差。',taiwan_supply_chain_link:'該股位於台灣鋼鐵、水泥或建材鏈，對國際報價、關稅與工程需求敏感。',why_this_stock:'它用來驗證原物料或政策訊號是否真正轉成台灣循環產業的報價與出貨。',validation_signal:'盤中確認同族群報價敏感股同步，且原物料或關稅訊號沒有反轉。'};
  if(industryCode==='BIOTECH_HEALTHCARE')return{first_order_impact:'臨床、法規、授權或醫療政策事件先改變產品成功率與商業化預期。',second_order_impact:'事件必須有公司公告或主管機關來源，才可傳到研發里程碑與營收預期。',taiwan_supply_chain_link:'該股位於台灣生技醫療鏈，受臨床進度、法規核准、授權與醫療需求影響。',why_this_stock:'它只在有公司或主管機關證據時作為事件標的，不用泛生技題材推論。',validation_signal:'確認公司公告或主管機關來源仍有效，盤中量價反應不得只是一檔無量上漲。'};
  if(industryCode==='DEFENSE_DRONE')return{first_order_impact:'國防政策、採購或地緣事件先改變軍工與無人機訂單預期。',second_order_impact:'採購預算與得標資訊必須可驗證，訊號才會傳到台灣航太、零組件與系統整合。',taiwan_supply_chain_link:'該股位於台灣軍工、航太或無人機鏈，受政策預算與具體訂單影響。',why_this_stock:'它用來驗證地緣事件是否真的形成台灣供應鏈訂單，而不是只靠新聞熱度。',validation_signal:'確認政策、採購或公司公告來源，並觀察軍工族群至少兩檔同步與量能擴大。'};
  if(industryCode==='CYBERSECURITY_SOFTWARE'||industryCode==='GAMING_CONTENT')return{first_order_impact:'政策、資安事件、企業 IT 支出或內容產品事件先改變訂單與營收預期。',second_order_impact:'客戶採用、產品上線或合約資訊必須可驗證，訊號才會傳到台灣軟體與內容公司。',taiwan_supply_chain_link:'該股位於台灣資安、軟體或數位內容鏈，受企業支出與產品事件影響。',why_this_stock:'它只在可追溯產品、合約或政策事件下成立，不以泛科技題材作推薦。',validation_signal:'確認公司或政策來源，並觀察同類股是否同步、成交量是否支持事件延續。'};
...
function enrichV10PremiumStockRecord(value:Record<string,unknown>):Record<string,unknown>{
  const symbol=String(value.symbol??value.stock_id??'');
  const sector=String(value.industry_name??value.sector??value.group??'對應產業');
  const profile=stockResearchProfile(symbol,sector,String(value.industry_code??''));
  const trigger=String(value.trigger_event??value.evidence_source??'').trim();
  const dataBasis=String(value.data_basis??'').trim();
  const sourceRefs=dataBasis.split('；').map((item)=>item.trim()).filter(Boolean);
  const transmission=[
    trigger?`事件來源：${trigger}`:'',
    profile.first_order_impact,
    profile.second_order_impact,
    profile.taiwan_supply_chain_link,
    profile.why_this_stock,
  ].filter(Boolean).join(' ');
  return {
    ...value,
    first_order_impact:profile.first_order_impact,
    second_order_impact:profile.second_order_impact,
    taiwan_supply_chain_link:profile.taiwan_supply_chain_link,
    why_this_stock:profile.why_this_stock,
    transmission_logic:transmission,
    reason_chain:[trigger,profile.first_order_impact,profile.second_order_impact,profile.taiwan_supply_chain_link,profile.why_this_stock].filter(Boolean),
    intraday_validation:`${profile.validation_signal} 同時確認 ${symbol} 相對 TAIEX 強弱與 ${sector} 族群至少兩檔同步。`,
    validation_signal:profile.validation_signal,
    invalidation_condition:`若 ${symbol} 弱於 TAIEX、${sector} 沒有量價同步，或事件來源更新後不再支持原假設，今日受惠判斷失效。`,
    source_refs:sourceRefs,
...
function normalizeBeneficiaryExplanation(stock:Record<string,unknown>):Record<string,unknown>{
  const trigger=String(stock.trigger_event||stock.catalyst||'').trim();
  const first=String(stock.first_order_impact||'').trim();
  const second=String(stock.second_order_impact||'').trim();
  const supply=String(stock.taiwan_supply_chain_link||'').trim();
  const why=String(stock.why_this_stock||'').trim();
  const current=String(stock.reason||stock.trigger_reason||'').trim();
  const chain=[trigger,first,second,supply,why].filter(Boolean);
  const reason=chain.length>=3?chain.slice(0,4).join(' '):current;
  const watchPoint=String(stock.watch_point||stock.validation_signal||'').trim();
  const invalidation=String(stock.risk_note||stock.invalidation_condition||'').trim();
  return{...stock,reason:reason||current,reason_chain:chain,data_basis:String(stock.data_basis||'').trim(),watch_point:watchPoint,validation_signal:String(stock.validation_signal||watchPoint),risk_note:invalidation,invalidation_condition:String(stock.invalidation_condition||invalidation)};
}
function finalSanitizeTWStocks(stocks:Record<string,unknown>[],_fallback:Record<string,unknown>[],label:string,log:(m:string)=>void):Record<string,unknown>[]{
  ...
  for(const s of stocks){const sym=String(s.symbol||s.stock_id||'');if(isValidTaiwanStock(s.symbol||s.stock_id,s.name||s.stock_name)&&!seen.has(sym)){const normalized=normalizeBeneficiaryExplanation(s);result.push({...normalized,not_buy_signal:true,source_type:normalized.source_type||'sanitized_tw_only'});seen.add(sym);}else if(sym){removedCount++;log('[finalSanitizeTWStocks:'+label+'] REMOVED: '+sym+' | '+(s.name||s.stock_name||''))}}
...
async function fetchSectorRotationForDate(supabase:RuntimeClient,scoreDate:string,log:(m:string)=>void):Promise<SectorRotationRow[]>{
  try{const r=await supabase.from('sector_rotation_scores').select('score_date,sector,sub_sector,rotation_score,direction,signal_label,leading_symbols,lagging_symbols,summary').eq('score_date',scoreDate).order('rotation_score',{ascending:false}).limit(10);const{data,error}=safeUnwrap<Record<string,unknown>[]>(r,log,'sector_rotation');if(error||!data?.length){log('[fetchSectorRotationForDate] no data for '+scoreDate);return [];}const rows:SectorRotationRow[]=data.map(function(row){return{score_date:row.score_date?String(row.score_date):undefined,sector:String(row.sector||''),sub_sector:String(row.sub_sector||''),rotation_score:Number(row.rotation_score)||0,direction:String(row.direction||''),signal_label:String(row.signal_label||''),leading_symbols:Array.isArray(row.leading_symbols)?row.leading_symbols.map(String):[],lagging_symbols:Array.isArray(row.lagging_symbols)?row.lagging_symbols.map(String):[],summary:row.summary?String(row.summary):undefined}});log('[fetchSectorRotationForDate] got '+rows.length+' sectors for '+scoreDate);return rows;}catch(e){log('[fetchSectorRotationForDate] exception: '+(e instanceof Error?e.message:String(e)));return [];}
}

async function reconstructResearchSectorRotationForDate(
  ...
  log('SECTOR_RECONSTRUCTION_READY date='+scoreDate+' batch='+result.batch_id+' sectors='+result.rows.length);
  return result.rows;
}
...
const SCENARIO_TRIGGERS:Record<string,{trigger_label:string;trigger_description:string;activation_check:(md:MarketIndicator[],dScore:MarketDataScore)=>boolean;stocks:{symbol:string;reason:string;risk_note:string;confidence_level:'high'|'medium'|'low'}[]}>={
  ...
  SEMI_EQUIPMENT:{trigger_label:'半導體設備轉強情境',trigger_description:'若全球半導體資本支出展望上修，設備族群可能領先反應',activation_check:function(md,dScore){const sox=md.find(function(m){return m.symbol.toUpperCase()==='SOX'||m.symbol.toUpperCase()==='PHLX'});const amat=md.find(function(m){return m.symbol.toUpperCase()==='AMAT'});const lrcx=md.find(function(m){return m.symbol.toUpperCase()==='LRCX'});return Boolean((sox&&sox.changePercent>1.0)&&(amat&&amat.changePercent>0||lrcx&&lrcx.changePercent>0));},stocks:[{symbol:'3037',reason:'半導體設備支出升溫帶動IC載板需求，欣興為台股載板龍頭',risk_note:'若資本支出上修僅為單一公司而非產業趨勢則效應有限',confidence_level:'low'},{symbol:'8046',reason:'ABF載板受惠先進封裝與HPC需求，南電為台灣主要供應商',risk_note:'載板族群若無法與半導體設備股同步走強則動能不足',confidence_level:'low'},]},
};
...
function isGenericSubscriberConclusion(value:unknown):boolean{
  const text=String(value||'').trim();
  if(text.length<24)return true;
  return /投資人應(?:密切)?(?:留意|關注|觀察)|需密切(?:留意|關注|觀察)|市場(?:動向|動態|情況)(?:將取決於|顯示)|後續(?:仍需)?觀察|市場仍有不確定性/.test(text);
}
function sanitizeMemberResearchNoteV2(note:unknown,fallback:MemberResearchNoteV2,log:(m:string)=>void):MemberResearchNoteV2{
  ...
  return{
    overnight_chain:Array.isArray(n.overnight_chain)&&n.overnight_chain.length>=1?n.overnight_chain as Record<string,unknown>[]:fallback.overnight_chain,
    ...
    intraday_validation:Array.isArray(n.intraday_validation)&&n.intraday_validation.length>=3?n.intraday_validation as Record<string,unknown>[]:fallback.intraday_validation,
    invalidation_rules:Array.isArray(n.invalidation_rules)&&n.invalidation_rules.length>=2?n.invalidation_rules as Record<string,unknown>[]:fallback.invalidation_rules,
    ...
    subscriber_value_sentence:isGenericSubscriberConclusion(n.subscriber_value_sentence)?fallback.subscriber_value_sentence:String(n.subscriber_value_sentence),
...
function findMarketIndicator(md:MarketIndicator[],syms:string[]):MarketIndicator|null{return findIndicator(md,syms);}
...
function sanitizeUnsupportedResearchText(text:string,txfAvailable:boolean):string{
  ...
  out=sanitizeUnsupportedAbsolutePriceLevels(out);
...
function calculateMemberValueScore(ai:Record<string,unknown>,dataQuality:string,note:Record<string,unknown>,stocks:Record<string,unknown>[],freshNewsCount:number):number{
  let score=20;
...
function enforceMemberResearchIntegrity(ai:Record<string,unknown>,dataQuality:string,md:MarketIndicator[],log:(m:string)=>void,freshNewsCount:number=0):Record<string,unknown>{
  ...
  const stocks=(v10Enabled&&Array.isArray(ai.today_beneficiary_stocks_v10)?ai.today_beneficiary_stocks_v10:Array.isArray(ai.beneficiary_stocks)?ai.beneficiary_stocks:Array.isArray(ai.today_beneficiary_stocks)?ai.today_beneficiary_stocks:[]) as Record<string,unknown>[];
  const reasoning=Array.isArray(note.beneficiary_reasoning)?note.beneficiary_reasoning as Record<string,unknown>[]:[];
  const candidates=Array.isArray(note.beneficiary_candidates)?note.beneficiary_candidates as Record<string,unknown>[]:[];
  if(v10Enabled){
    reasoning.length=0;
    candidates.length=0;
    note.first_beneficiary_stock=null;
  }
  for(const stock of stocks){
    ...
    const sector=String(stock.industry_name||stock.sector||stock.group||catalystTypeForStock(symbol));
    const trigger=String(stock.trigger_event||stock.catalyst||'市場資料與類股輪動');
    const reason=String(stock.transmission_logic||stock.reason||stock.why_this_stock||stock.member_thesis||`${trigger} → ${sector}供應鏈 → ${name}；成立與否以盤中相對強弱和族群同步性驗證。`).trim();
    const validation=String(stock.intraday_validation||stock.validation_signal||stock.watch_point||`09:30 確認 ${name} 是否優於 TAIEX 且同族群同步`);
  ...
  if(v10Enabled&&orderedCandidates.length>0){
    const first=orderedCandidates[0];
    note.first_beneficiary_stock={stock_code:first.stock_code,stock_name:first.stock_name,sector:first.sector,benefit_source:first.trigger_event,relationship_to_thesis:first.why_this_stock||first.reason,validation_signal:first.validation_signal,invalidation_condition:first.invalidation_condition||first.risk,source_signals:first.source_signals||first.evidence};
  }
  ...
  for(const key of ['today_quote','free_summary','line_push_copy','today_beneficiary_stocks','beneficiary_stocks','v8_beneficiary_chain','v8_overnight_causal_chain','v8_daily_sentence','causal_overnight_impact_chains','intraday_tracking_plan']){
    if(ai[key]!==undefined)ai[key]=sanitizeResearchValue(ai[key],txfAvailable);
  }
  const memberValueScore=calculateMemberValueScore(ai,dataQuality,note,stocks,freshNewsCount);
  ai.member_value_score=memberValueScore;
  ai.member_value_score_semantics='fresh evidence + actionable daily sentence + intraday/invalidation structure + complete recommendation reasoning or evidence-backed no-trade discipline';
...
// Match Fetch's mandatory PREMARKET inputs: all three US references are
// required. This is independent of optional company/recommendation inputs.
function checkMVPStatus(md:MarketIndicator[],_log:(m:string)=>void):MVPStatus{const n=Boolean(findIndicator(md,['NVDA'])),t=Boolean(findIndicator(md,['TSM'])),p=Boolean(findIndicator(md,['SPX']));const c=[n,t,p].filter(Boolean).length;return{nvdaPresent:n,tsmPresent:t,spxPresent:p,mvpCount:c,mvpInsufficient:c<3}}
function checkTWCoreStatus(md:MarketIndicator[],_log:(m:string)=>void):TWCoreStatus{const taiex=Boolean(findIndicator(md,['TAIEX','TWII','^TWII']));const txf=Boolean(findIndicator(md,['TXF','TX','MTX']));const ts=Boolean(findIndicator(md,['2330','2330.TW']));const mm=[taiex,txf,ts].filter(function(v){return !v}).length;return{taiexPresent:taiex,txfPresent:txf,ts2330Present:ts,missingCount:mm,dataInsufficient:mm>=2}}

function failClosedForMissingRequiredEvidence(
  ai:Record<string,unknown>,
  missingSources:string[],
  log:(m:string)=>void,
):Record<string,unknown>{
  const warning='核心市場證據不足，本日僅保存降級報告，不提供方向或個股建議。';
  const note=(ai.member_research_note_v2&&typeof ai.member_research_note_v2==='object'&&!Array.isArray(ai.member_research_note_v2)?{...(ai.member_research_note_v2 as Record<string,unknown>)}:{});
  const v8Daily=(ai.v8_daily_sentence&&typeof ai.v8_daily_sentence==='object'&&!Array.isArray(ai.v8_daily_sentence)?{...(ai.v8_daily_sentence as Record<string,unknown>)}:{});
  const freeSummary=(ai.free_summary&&typeof ai.free_summary==='object'&&!Array.isArray(ai.free_summary)?{...(ai.free_summary as Record<string,unknown>)}:{});
  const lineCopy=(ai.line_push_copy&&typeof ai.line_push_copy==='object'&&!Array.isArray(ai.line_push_copy)?{...(ai.line_push_copy as Record<string,unknown>)}:{});
  const publishGate=(ai.content_publish_gate&&typeof ai.content_publish_gate==='object'&&!Array.isArray(ai.content_publish_gate)?{...(ai.content_publish_gate as Record<string,unknown>)}:{});
  const blockingIssues=Array.isArray(publishGate.blocking_issues)?publishGate.blocking_issues.map(String):[];
  log('REQUIRED_MARKET_EVIDENCE_INSUFFICIENT missing_sources='+missingSources.join(','));
  return{
    ...ai,
    market_bias:'資料不足',
    data_status:'insufficient',
    data_quality:'degraded',
    required_market_evidence_status:'insufficient',
    required_market_evidence_missing:missingSources,
    publish_ready:false,
    today_quote:warning,
    today_beneficiary_stocks:[],
    beneficiary_stocks:[],
    core_beneficiary_stocks:[],
    extended_watchlist:[],
    scenario_watchlist:[],
    today_beneficiary_stocks_v10:[],
    v10_observation_watchlist:[],
    v10_risk_watchlist:[],
    v10_beneficiary_enabled:false,
    v10_candidate_count:0,
    v10_data_quality_status:'insufficient_market_evidence',
    v10_warning:warning,
    premium_content_eligible:false,
    member_research_note_v2:{...note,beneficiary_candidates:[],intraday_validation:[],data_status:'insufficient',subscriber_value_sentence:warning,today_core_thesis:warning},
    v8_daily_sentence:{...v8Daily,status:'insufficient',sentence:warning,logic_source:[],decision_mode:'blocked'},
    free_summary:{...freeSummary,one_sentence:warning,market_bias:'資料不足'},
    line_push_copy:{...lineCopy,one_sentence:warning,market_bias:'資料不足'},
    content_publish_gate:{...publishGate,overall_status:'拒絕發布',blocking_issues:Array.from(new Set([...blockingIssues,'required_market_evidence_missing']))},
  };
}
...
function calculateMarketDataScore(md:MarketIndicator[]):MarketDataScore{
  ...
  const f=(syms:string[])=>findIndicator(md,syms);
...
function hasAnySymbol(md:MarketIndicator[],syms:string[]):boolean{return Boolean(findIndicator(md,syms));}
...
function extractMarketNumericPayload(md:MarketIndicator[]):Record<string,unknown>{
  const f=(syms:string[])=>findIndicator(md,syms);const p:Record<string,unknown>={};
  const vix=f(['VIX','VIXINDEX']);if(vix&&Number.isFinite(vix.value))p.vix=vix.value;
  const sp=f(['SPX','SP500','GSPC']);if(sp&&Number.isFinite(sp.changePercent))p.sp500_change=sp.changePercent;
  const sox=f(['SOX','PHLX']);if(sox&&Number.isFinite(sox.changePercent))p.sox_change=sox.changePercent;
  const ndq=f(['IXIC','NASDAQ']);if(ndq&&Number.isFinite(ndq.changePercent))p.nasdaq_change=ndq.changePercent;
  const dji=f(['DJI','DOW','DJIA']);if(dji&&Number.isFinite(dji.changePercent))p.dow_change=dji.changePercent;
  const dx=f(['DXY','USDINDEX']);if(dx&&Number.isFinite(dx.value))p.dxy=dx.value;
  const uy=f(['US10Y','TNX','T10Y']);if(uy&&Number.isFinite(uy.value))p.us10y_yield=uy.value;
  const tm=f(['TSM','TSMC']);if(tm&&Number.isFinite(tm.changePercent))p.tsm_adr_change=tm.changePercent;
  const txf=f(['TXF','TX','MTX']);if(txf&&Number.isFinite(txf.changePercent))p.taiwan_futures_change=txf.changePercent;
  return p;
}

async function fetchMarketData(supabase:RuntimeClient,log:(msg:string)=>void){
  const empty={marketData:[] as MarketIndicator[],latestDataTime:null as Date|null,isStale:true,dataCount:0,rawDataCount:0,invalidNumericSources:[] as string[]};
  try{
    const [canonicalResult,legacyResult]=await Promise.all([
      supabase.from('market_quotes').select('symbol,name,market,value,change_value,change_percent,captured_at,provider,freshness_status,quality_status').lte('captured_at',new Date().toISOString()).order('captured_at',{ascending:false}).limit(60),
      supabase.from('market_data').select('*').lte('captured_at',new Date().toISOString()).order('captured_at',{ascending:false}).limit(60),
    ]);
    const canonical=safeUnwrap<Record<string,unknown>[]>(canonicalResult,log,'market_quotes');
    const legacy=safeUnwrap<Record<string,unknown>[]>(legacyResult,log,'market_data_fallback');
    const rows=mergeCanonicalAndLegacyMarketRows(canonical.data||[],legacy.data||[]);
    if(rows.length===0){log('fmData:empty canonical_error='+(canonical.error||'none')+' legacy_error='+(legacy.error||'none'));return empty;}
    const normalized=normalizeMarketDataRows(rows,Date.now(),HOURS_24);
    if(normalized.invalidNumericSources.length>0)log('MARKET_DATA_INVALID_NUMERIC '+normalized.invalidNumericSources.join(','));
    log('MARKET_DATA_CONTRACT canonical_rows='+(canonical.data?.length||0)+' legacy_rows='+(legacy.data?.length||0)+' merged_symbols='+rows.length);
    return normalized;
  }catch(e){log('fmData exc:'+(e instanceof Error?e.message:String(e)));return empty;}
}

async function fetchMarketNews(supabase:RuntimeClient,log:(msg:string)=>void):Promise<FetchNewsResult>{
  try{
    const eventResult=await supabase.from('news_events')
      .select('id,title,summary,source_name,source_url,published_at,created_at,event_type,symbols,sectors,freshness_status,fingerprint,raw_payload')
      .in('freshness_status',['fresh','recent'])
      .order('published_at',{ascending:false})
      .limit(100);
    const{data,error}=safeUnwrap<Record<string,unknown>[]>(eventResult,log,'news_events');
    if(error||!data?.length){log('canonicalNews:empty');return{newsData:[],latestNewsTime:null,isStale:true,newsCount:0}}
    const recent=filterRecentNewsRows(data,Date.now(),48).slice(0,30);
    if(recent.length===0){log('canonicalNews:no rows published within 48h');return{newsData:[],latestNewsTime:null,isStale:true,newsCount:0}}
    const fingerprints=recent.map(function(row){return String(row.fingerprint||'');}).filter(Boolean);
    const tagResult=fingerprints.length>0
      ? await supabase.from('news_event_tags').select('news_key,event_type,related_sectors,related_symbols,related_global_symbols,impact_direction,impact_strength,confidence_score,reasoning,tagged_at').in('news_key',fingerprints)
      : {data:[],error:null};
    const tagsByKey=new Map<string,Record<string,unknown>>();
    if(tagResult.error){log('canonicalNews:tag query failed '+tagResult.error.message);return{newsData:[],latestNewsTime:null,isStale:true,newsCount:0}}
    for(const tag of tagResult.data||[])tagsByKey.set(String(tag.news_key||''),tag as Record<string,unknown>);
    const tagged=recent.filter(function(row){return tagsByKey.has(String(row.fingerprint||''));});
    if(tagged.length===0){log('canonicalNews:no catalyst-tagged events');return{newsData:[],latestNewsTime:null,isStale:true,newsCount:0}}
    let latest:Date|null=null;
    for(const row of tagged){const dt=new Date(String(row.published_at||''));if(!Number.isNaN(dt.getTime())&&(!latest||dt>latest))latest=dt}
    return{newsData:tagged.map(function(row){
      const tag=tagsByKey.get(String(row.fingerprint||''))||{};
      const rawPayload=canonicalRecord(row.raw_payload);
      return{
        id:String(row.id||''),title:String(row.title||''),source:String(row.source_name||''),url:String(row.source_url||''),
        published_at:row.published_at?String(row.published_at):null,created_at:String(row.created_at||''),
        related_sectors:Array.isArray(tag.related_sectors)?tag.related_sectors.map(String):Array.isArray(row.sectors)?row.sectors.map(String):null,
        related_symbols:Array.isArray(tag.related_symbols)?tag.related_symbols.map(String):Array.isArray(row.symbols)?row.symbols.map(String):null,
        event_type:canonicalText(tag.event_type,row.event_type)||null,
        taiwan_impact_summary:canonicalText(rawPayload.taiwan_impact_summary,row.summary)||null,
        raw_payload:{...rawPayload,canonical_event_id:row.id,canonical_fingerprint:row.fingerprint,catalyst_tag:tag},
      };
    }),latestNewsTime:latest,isStale:!latest||Date.now()-latest.getTime()>HOURS_24,newsCount:tagged.length};
  }catch(e){log('canonicalNews exc:'+(e instanceof Error?e.message:String(e)));return{newsData:[],latestNewsTime:null as Date|null,isStale:true,newsCount:0}}
}

async function fetchPreviousReportForDate(supabase:RuntimeClient,reportDate:string,log:(msg:string)=>void):Promise<Record<string,unknown>|null>{try{const result=await supabase.from('reports').select('report_date,market_bias,confidence_score,ai_strategy_json,created_at,updated_at').eq('report_date',reportDate).order('created_at',{ascending:false}).limit(1).maybeSingle();const {data,error}=safeUnwrap<Record<string,unknown>>(result,log,'previous_report');if(error||!data){log(`previous_report:empty ${reportDate}`);return null;}return data;}catch(e){log(`previous_report_exception:${e instanceof Error?e.message:String(e)}`);return null;}}

async function fetchRecentReportsForV10Universe(supabase:RuntimeClient,beforeDate:string,log:(msg:string)=>void):Promise<Record<string,unknown>[]> {try{const result=await supabase.from('reports').select('report_date,ai_strategy_json').lt('report_date',beforeDate).order('report_date',{ascending:false}).limit(14);const {data,error}=safeUnwrap<Record<string,unknown>[]>(result,log,'v10_recent_reports');if(error||!data?.length){log(`v10_recent_reports:empty before ${beforeDate}`);return [];}return data;}catch(e){log(`v10_recent_reports_exception:${e instanceof Error?e.message:String(e)}`);return [];}}
...
function buildDeterministicAIStrategyJson(md:MarketIndicator[],newsData:MarketNewsItem[],todayDate:string,dates:{twCoreDate:string;usGlobalDate:string;dataTimeBasis:string},dScore:MarketDataScore,twStatus:TWCoreStatus,mvpStatus:MVPStatus,reportMode:string,sectorData:SectorRotationRow[],log:(m:string)=>void,confidenceResult?:ReportConfidenceScore):Record<string,unknown>{
  ...
  return{
    ...
    line_push_copy:buildGuardedLinePushCopy(todayDate,marketBias,confidenceScore,md,applyBiasGuardrails(md,dScore.baseScore,dates),dates)
  };
}
...
function buildOpenAIUserPrompt(md:MarketIndicator[],newsData:MarketNewsItem[],todayDate:string,dates:{twCoreDate:string;usGlobalDate:string},sectorContextSummary:string,allowedCandidateRows:Record<string,unknown>[]=[]):string{
  const mdLines=md.map(function(m){return 'evidence_ref=market_data.'+m.symbol.toUpperCase()+' | '+safeOpenAIMarketDataLine(m);}).join('\n');
  const newsLines=newsData.slice(0,12).map(function(n,idx){return 'evidence_ref=market_news.'+(n.id||String(idx+1))+' | '+n.title+' | '+(n.taiwan_impact_summary||'')}).join('\n');
  const allowedCandidateLines=allowedCandidateRows.map(function(row,idx){const r=row as Record<string,unknown>;const symbol=String(r.symbol??r.stock_id??'');return String(idx+1)+'. evidence_ref=allowed_candidate.'+symbol+' | '+symbol+' '+String(r.name??r.stock_name??'')+' | '+String(r.sector??r.category??'')+' | level='+String(r.beneficiary_level??'')+' | reason='+String(r.reason??'')+' | data_basis='+String(r.data_basis??'');}).join('\n');
...
function isSyntheticDailySentence(value:string):boolean{const text=value.replace(/\s+/g,' ').trim();if(!text)return true;return /基準日期|信心\s*\d+\s*\/\s*100/.test(text)||/^\d{4}-\d{2}-\d{2}(?:未|[，,。；;：:\s])/.test(text)||/^(偏多|偏空|偏弱|偏強|中性|震盪)觀察[，,。\s]/.test(text);}
function dailySentenceFingerprint(value:string):string{return value.toLowerCase().replace(/[+-]?\d+(?:\.\d+)?%?/g,'#').replace(/[\s，,。；;：:「」『』（）()]/g,'').trim();}
function resolveEvidenceBackedDailySentence(ai:Record<string,unknown>,md:MarketIndicator[],dates:MarketFreshnessDates|undefined,marketBias:string,previousSentence:string=''):string{
  const v8=(ai.v8_daily_sentence&&typeof ai.v8_daily_sentence==='object'&&!Array.isArray(ai.v8_daily_sentence)?ai.v8_daily_sentence:{} ) as Record<string,unknown>;
  const line=(ai.line_push_copy&&typeof ai.line_push_copy==='object'&&!Array.isArray(ai.line_push_copy)?ai.line_push_copy:{} ) as Record<string,unknown>;
  const note=(ai.member_research_note_v2&&typeof ai.member_research_note_v2==='object'&&!Array.isArray(ai.member_research_note_v2)?ai.member_research_note_v2:{} ) as Record<string,unknown>;
  const free=(ai.free_summary&&typeof ai.free_summary==='object'&&!Array.isArray(ai.free_summary)?ai.free_summary:{} ) as Record<string,unknown>;
  const txf=findIndicator(md,['TXF','TX','MTX','TXF1']);
  const txfAvailable=Boolean(txf&&!isCoreMarketDataStale(txf,dates));
  ...
  const definitions:[string[],string,string,string][]=[
    ...
    [['SOX','PHLX'],'SOX','半導體','2330 與半導體族群是否同向'],
    [['NVDA'],'NVDA','AI 伺服器','AI 伺服器代表股是否同步放量'],
    [['IXIC','NASDAQ'],'NASDAQ','成長股','電子權值是否承接隔夜風險偏好'],
    [['TXF','TX','MTX'],'台指期','台股方向','台指期與加權指數是否同向'],
  ];
  for(const [symbols,label,transmission,validation] of definitions){const indicator=findIndicator(md,symbols);if(!indicator||isCoreMarketDataStale(indicator,dates))continue;const direction=indicator.changePercent>=0?'提供支撐':'形成壓力';const text=label+' '+fmtSignedPct(indicator.changePercent)+' 對'+transmission+direction+'；09:30 先看'+validation+'，未確認前不追價。';if(isNovel(text))return text;}
  return marketBias+'；09:30 先看加權指數、2330 與主要族群是否同向，證據未同步前不追價。';
}

function finalizeMarketNarrativeInput(ai:Record<string,unknown>,md:MarketIndicator[],dates:MarketFreshnessDates|undefined,marketBias:string,previousSentence:string=''):Record<string,unknown>{
  const sentence=resolveEvidenceBackedDailySentence(ai,md,dates,marketBias,previousSentence);
  const news=Array.isArray(ai.important_news)?ai.important_news as Record<string,unknown>[]:[];
  const transmission=deriveEvidenceBackedTaiwanTransmission(ai,news);
  return {...ai,today_quote:sentence,
    v8_daily_sentence:{...canonicalRecord(ai.v8_daily_sentence),sentence},
    free_summary:{...canonicalRecord(ai.free_summary),one_sentence:sentence},
    ...(transmission?{taiwan_transmission:transmission,member_research_note_v2:{...canonicalRecord(ai.member_research_note_v2),taiwan_transmission:transmission}}:{}),
  };
}

function buildImportantNewsPayload(newsData:MarketNewsItem[]):Record<string,unknown>[] {
  return newsData.slice(0,8).map(function(news){
    return {
      id:news.id,
      title:news.title,
      source:news.source,
      ...
      canonical_event_id:news.id,
      event_type:news.event_type,
      related_sectors:news.related_sectors||[],
      related_symbols:news.related_symbols||[],
      catalyst_tag:canonicalRecord(news.raw_payload?.catalyst_tag),
      taiwan_impact_summary:news.taiwan_impact_summary||'',
    };
  });
}

function applyFreshNewsEvidenceGate(ai:Record<string,unknown>,importantNews:Record<string,unknown>[]):Record<string,unknown>{
  const next={...ai};
  ...
  const evidenceQuality=(next.content_evidence_quality&&typeof next.content_evidence_quality==='object'&&!Array.isArray(next.content_evidence_quality)?next.content_evidence_quality:{} ) as Record<string,unknown>;
  ...
  gate.fresh_news_status=importantNews.length>0?'verified_news_available':verifiedMarketCount>0?'verified_market_evidence_available':'absent_market_evidence_required';
...
function buildCanonicalDecisionPayload(
  ...
  const reasons=canonicalRecords(marketSections.supporting_evidence)
    .map(function(reason){ return canonicalText(reason.statement); })
    .filter(Boolean)
    .slice(0,3);
  const preferredSectors=canonicalRecords(canonicalRecord(marketSections.transmission_narrative).path)
    .filter(function(node){return node.stage==='industry';})
    .map(function(node){return canonicalText(node.subject);})
    .filter(Boolean)
    .slice(0,3);
  const invalidationRules=canonicalRecords(canonicalRecord(marketSections.failure_scenario).triggers)
    .map(function(trigger){return{condition:trigger.condition,meaning:trigger.meaning,
      evidence_refs:trigger.evidence_required,action_note:canonicalRecord(marketSections.failure_scenario).stop_action};}).slice(0,8);
  const sourceRefs=canonicalMarketSourceRefs(ai);
  ...
  const publishable=marketGate.eligible;
  ...
  return{
    ...
    data_as_of:canonicalText(researchMaster.data_as_of,ai.data_as_of)||null,
    generated_at:canonicalText(ai.generated_at),
    engine_version:canonicalText(ai.version,VERSION)+':'+RESEARCH_PIPELINE_VERSION,
    market_score:presentNumber(ai.market_bias_score),
    confidence_score:confidenceScore,
    coverage_score:researchQuality.evidence_coverage,
    action:canonicalDecisionAction(decisionMode),
    ...
    source_freshness:{
      data_quality:canonicalText(ai.data_quality),
      missing_sources:Array.isArray(ai.missing_sources)?ai.missing_sources:[],
      canonical_news_events:importantNews.map(function(news){return{
        id:news.canonical_event_id,
        published_at:news.published_at,
        event_type:news.event_type,
        catalyst_tagged:Boolean(canonicalRecord(news.catalyst_tag).event_type),
      };}),
    },
    source_refs:sourceRefs,
    generated_text:{
      ...
      missing_sources:ai.missing_sources,
      report_date:marketGate.report_date,
      report_status:marketGate.report_status,
      recommendation_status:marketGate.recommendation_status,
      recommendation_gate:marketGate.recommendation_gate,
      market_report_gate:marketGate,
      daily_sentence:publishable?canonicalText(canonicalRecord(marketSections.executive_summary).text):blockedDailySentence,
      market_bias:marketBias,
      confidence_score:confidenceScore,
      reasons:publishedReasons,
      preferred_sectors:publishedSectors,
      recommendations:publishedRecommendations,
      do_not_do:publishable?(Array.isArray(canonicalRecord(marketSections.decision_guide).avoid_actions)
        ?(canonicalRecord(marketSections.decision_guide).avoid_actions as unknown[]).filter((item):item is string=>typeof item==='string').join('；'):''):'不要把未達證據標準的觀察名單當成推薦或追價理由。',
      invalidation_conditions:publishable?invalidationRules:[],
      next_checkpoint:'09:30',
    },
    input_coverage:{data_quality:ai.data_quality,source_breakdown:canonicalRecord(ai.source_breakdown),research_quality:researchQuality},
    missing_sources:Array.isArray(ai.missing_sources)?ai.missing_sources:[],
    content_score:contentReview.score,
    content_grade:contentReview.grade,
    content_score_breakdown:contentReview.breakdown,
    reason_codes:marketGate.reason_codes,
...
async function publishCanonicalDecisionSnapshot(
  ...
  try{
    const result=await supabase.rpc('publish_decision_snapshot_v3',{
      p_report_date:reportDate,
      p_session_type:'PREMARKET',
      p_report_id:reportId,
      p_payload:payload,
      p_correlation_id:correlationId,
      p_idempotency_key:'production-report:'+reportDate+':PREMARKET',
      p_attempt:Math.max(1,attempt),
    });
...
type CanonicalMemberRevisionResult={
  revisionId:string|null;
  eligible:boolean;
  status:string;
  reasonCodes:string[];
  canonicalContract:Record<string,unknown>|null;
};

async function publishCanonicalMemberRevision(
  supabase:RuntimeClient,
  reportDate:string,
  reportId:string,
  snapshotId:string,
  ...
  try{
    const snapshotResult=await supabase.from('decision_snapshots').select('id,report_id,report_date,version,status,action,decision_mode,generated_text,source_refs,snapshot_fingerprint,content_score').eq('id',snapshotId).maybeSingle();
    ...
    const semanticResult=evaluateCanonicalSemanticCoherenceGate({
      ...
      sections:{
        public_thesis:canonicalText(canonicalRecord(snapshot.generated_text).daily_sentence),
        member_thesis:canonicalText(memberContent.today_core_thesis),
        taiwan_transmission:memberContent.taiwan_transmission,
        line_summary:memberContent.line_summary,
      ...
      quality_counters:{
        ...
        duplicate_claim_count:qualityGate.duplicate_claim_count,
        missing_section_count:qualityGate.missing_section_count,
      },
      evidence_coverage:evidenceCoverage,
      content_score:contentScore,
      checked_at:new Date().toISOString(),
    }) as Record<string,unknown>;
    const rpcResult=await supabase.rpc('publish_member_content_revision_v1',{
      p_report_date:reportDate,
      p_report_id:reportId,
      p_decision_snapshot_id:snapshotId,
      p_idempotency_key:idempotencyKey,
      p_source_revision:sourceRevision,
      p_canonical_contract:canonicalContract,
      p_member_content:memberContent,
      p_semantic_result:semanticResult,
      p_content_score:contentScore,
      p_evidence_coverage:evidenceCoverage,
      p_generated_at:new Date().toISOString(),
      p_metadata:{actor,suppress_notifications:true,source_report_version:canonicalText(ai.version)},
    });
    const{data:revisionId,error:revisionError}=safeUnwrap<string>(rpcResult,log,'publishCanonicalMemberRevision');
    if(revisionError||!revisionId)return blocked('MEMBER_REVISION_PUBLISH_FAILED');
    const eligible=semanticResult.eligible===true;
    const reasonCodes=Array.isArray(semanticResult.reason_codes)?semanticResult.reason_codes.map(String):[];
    log('CANONICAL_MEMBER_REVISION '+String(semanticResult.status||'BLOCKED')+' revision_id='+revisionId+' reasons='+(reasonCodes.join('|')||'none'));
    return{revisionId:String(revisionId),eligible,status:String(semanticResult.status||'BLOCKED'),reasonCodes,canonicalContract};
  }catch(error){
    log('CANONICAL_MEMBER_REVISION_EXCEPTION '+(error instanceof Error?error.message:String(error)));
    return blocked('MEMBER_REVISION_EXCEPTION');
  }
}

async function recoverCanonicalMemberRevision(
  supabase:RuntimeClient,
  reportDate:string,
  idempotencyKey:string,
  log:(message:string)=>void,
  ...
  const [reportResult,snapshotResult]=await Promise.all([
    ...
    supabase.from('decision_snapshots').select('id,report_id,report_date,version,status,action,decision_mode,generated_text,source_refs,snapshot_fingerprint,content_score').eq('report_date',reportDate).eq('session_type','PREMARKET').eq('is_current',true).order('version',{ascending:false}).limit(1).maybeSingle(),
  ]);
  ...
  if(reportResult.error||snapshotResult.error||!report||!snapshot){
    return{revisionId:null,eligible:false,status:'BLOCKED',reasonCodes:['RECOVERY_SOURCE_NOT_FOUND'],canonicalContract:null,reportId:report?String(report.id):null,snapshotId:snapshot?String(snapshot.id):null};
  }
  const ai=canonicalRecord(report.ai_strategy_json);
  const sourceRevision=canonicalText(snapshot.snapshot_fingerprint,`${snapshot.version}:${report.updated_at}`);
  const result=await publishCanonicalMemberRevision(supabase,reportDate,String(report.id),String(snapshot.id),ai,idempotencyKey,sourceRevision,'ma-ops-safe-recovery',log);
  return{...result,reportId:String(report.id),snapshotId:String(snapshot.id)};
}

class PublicationQualityError extends Error {
  constructor(readonly reasonCodes:string[]){super('PUBLICATION_GATE_BLOCKED');}
}
type ResearchInputRun={id:string;fingerprint:string;attempt:number};

function projectReviewedMarketDecision(ai:Record<string,unknown>,decision:Record<string,unknown>,newsCount:number):Record<string,unknown>{
  const generated=canonicalRecord(decision.generated_text);
  const frozenSentence=canonicalText(canonicalRecord(canonicalRecord(canonicalMarketDocument(generated).sections).executive_summary).text);
  if(!frozenSentence||generated.daily_sentence!==frozenSentence
    ||JSON.stringify(generated.canonical_market_state)!==JSON.stringify(ai.canonical_market_state)){
    throw new PublicationQualityError(['CANONICAL_EDITORIAL_PROJECTION_MISMATCH']);
  }
  const projected=canonicalReportProjection(ai,decision);
  const frozenReview=evaluateMarketContentIntelligence(generated,newsCount);
  const projectedReview=evaluateMarketContentIntelligence(projected,newsCount);
  if(!frozenReview.publishable||!projectedReview.publishable
    ||frozenReview.score!==decision.content_score||projectedReview.score!==frozenReview.score
    ||JSON.stringify(frozenReview)!==JSON.stringify(projectedReview)
    ||JSON.stringify(frozenReview.breakdown)!==JSON.stringify(decision.content_score_breakdown)
    ||frozenReview.grade!==decision.content_grade
    ||JSON.stringify(frozenReview.reason_codes)!==JSON.stringify(decision.reason_codes)
    ||JSON.stringify(frozenReview.generic_flags)!==JSON.stringify(decision.generic_content_flags)){
    throw new PublicationQualityError(['CANONICAL_EDITORIAL_PROJECTION_MISMATCH']);
  }
  return projected;
}

async function writeReport(supabase:RuntimeClient,todayDate:string,aiStrategyJson:Record<string,unknown>,marketBias:string,rawConfidenceScore:number|null,reportMode:string,md:MarketIndicator[],newsData:MarketNewsItem[],log:(m:string)=>void,tdInfo?:TradingDayInfo,dates?:MarketFreshnessDates,previousDailySentence:string='',learningConfidence:ProductionLearningConfidence|null=null,correlationId:string=crypto.randomUUID(),recoveryAttempt:number=1,startedAt:string=new Date().toISOString(),inputRun?:ResearchInputRun):Promise<{reportId:string;snapshotId:string|null;memberRevision:CanonicalMemberRevisionResult}|null>{
  try{
    ...
    const importantNews=buildImportantNewsPayload(newsData);
    const verifiedMarketEvidence=md.map((item)=>normalizePremiumMarketEvidence(item)).filter((item):item is Record<string,unknown>=>Boolean(item)&&['fresh','recent'].includes(String(item?.freshness_status)));
    aiStrategyJson.content_evidence_quality={
      contract_version:'PREMIUM_EVIDENCE_V1',
      verified_news_count:importantNews.length,
      verified_market_count:verifiedMarketEvidence.length,
      verified_catalyst_count:importantNews.length+verifiedMarketEvidence.length,
      all_news_traceable:importantNews.every((item)=>Boolean(item.source)&&/^https?:\/\//i.test(String(item.url??''))&&Boolean(item.published_at)),
      blank_market_change_count:verifiedMarketEvidence.filter((item)=>!Number.isFinite(Number(item.change_percent))).length,
      rejected_market_row_count:md.length-verifiedMarketEvidence.length,
    };
    aiStrategyJson=applyFreshNewsEvidenceGate(aiStrategyJson,importantNews);
    ...
    // Trading-day prose was finalized and audited before CMS assembly. Do not
    // replace it after the ledger freezes (the old pre/postwrite 100→70 bug).
    if(!tradingDayInfo.is_trading_day)aiStrategyJson=finalizeMarketNarrativeInput(aiStrategyJson,md,dates,marketBias,previousDailySentence);
    const todayQuote=tradingDayInfo.is_trading_day
      ?canonicalText(canonicalRecord(canonicalRecord(canonicalMarketDocument(aiStrategyJson).sections).executive_summary).text)
      :canonicalText(aiStrategyJson.today_quote);
    ...
    const managedContentIssues=new Set(['content_score_below_80','content_score_below_90','recommendation_reasoning_incomplete','decision_mode_incomplete','generic_content_detected','evidence_quality_contract_missing','verified_catalyst_evidence_missing','news_traceability_incomplete','blank_market_change_detected']);
    const existingBlockingIssues=(Array.isArray(contentPublishGate.blocking_issues)?contentPublishGate.blocking_issues.map(String):[]).filter(function(issue){return!managedContentIssues.has(issue);});
    ...
    contentPublishGate.overall_status=contentReview.publishable?'可公開':contentReview.score>=70?'內容降級':'拒絕發布';
    ...
    const premiumGate=evaluatePremiumContentGate(aiStrategyJson,importantNews.length);
    aiStrategyJson.premium_content_gate={status:premiumGate.status,eligible:premiumGate.eligible,decision_mode:premiumGate.decision_mode,reason_codes:premiumGate.reason_codes,recommendation_count:premiumGate.recommendation_count,complete_recommendation_count:premiumGate.complete_recommendation_count,content_score:premiumGate.content_score,content_grade:premiumGate.content_grade};
    aiStrategyJson.premium_content_status=premiumGate.status;
    aiStrategyJson.premium_decision_mode=premiumGate.decision_mode;
    ...
    const abstentionDecision=resolveAbstentionDecision({
      ...
      confidence_score:confidenceForPolicy,
      evidence_count:importantNews.length+verifiedMarketEvidence.length,
      missing_critical_sources:criticalMissingSources,
    },RUNTIME_QUALITY_POLICY);
    const positiveMarketEvidence=md.filter(function(item){return Number(item.changePercent)>0;}).map(function(item){return item.symbol+':'+Number(item.changePercent).toFixed(2)+'%';});
    const negativeMarketEvidence=md.filter(function(item){return Number(item.changePercent)<0;}).map(function(item){return item.symbol+':'+Number(item.changePercent).toFixed(2)+'%';});
    const marketBreadth=md.length===0?0:((positiveMarketEvidence.length-negativeMarketEvidence.length)/md.length)*100;
    const averageTrend=md.length===0?0:md.reduce(function(sum,item){return sum+Number(item.changePercent||0);},0)/md.length*20;
    const vixRow=md.find(function(item){return String(item.symbol).toUpperCase()==='VIX';});
    const regime=classifyMarketRegime({
      trend_score:averageTrend,
      volatility_score:Math.min(100,Math.abs(Number(vixRow?.changePercent||0))*12),
      liquidity_score:coverageScore,
      breadth_score:marketBreadth,
    });
    const debate=buildBullBearDebate({
      supporting_evidence:[...positiveMarketEvidence,...importantNews.slice(0,3).map(function(item){return String(item.title||'');})],
      counter_evidence:negativeMarketEvidence,
      confidence_score:confidenceForPolicy,
      market_regime:regime,
      abstention:abstentionDecision,
    });
    const safeMode=abstentionDecision.safe_mode===true||canonicalRecord(aiStrategyJson.runtime_cost_guard).safe_mode===true;
    aiStrategyJson.runtime_policy_version=RUNTIME_QUALITY_POLICY.version;
    aiStrategyJson.abstention_decision=abstentionDecision;
    aiStrategyJson.market_regime_v1=regime;
    if(!aiStrategyJson.market_regime)aiStrategyJson.market_regime=regime;
    aiStrategyJson.bull_bear_debate=debate;
    ...
    if(tradingDayInfo.is_trading_day){
      ...
      // Provisional metadata is replaced inside the transaction with the actual
      // snapshot identity. It is never returned as a published revision.
      ...
      const bundle=await supabase.rpc('publish_research_bundle_v1',{
        ...
        p_decision:decisionPayload,p_contract:contract,p_member:member,p_semantic:semantic,
      });
      if(bundle.error)throw new Error('ATOMIC_PUBLICATION_FAILED: '+bundle.error.message);
      const published=canonicalRecord(bundle.data);
      if(published.success!==true||!published.report_id||!published.decision_snapshot_id||!published.member_content_revision_id)throw new Error('ATOMIC_PUBLICATION_RECEIPT_INVALID');
      return{reportId:String(published.report_id),snapshotId:String(published.decision_snapshot_id),memberRevision:{revisionId:String(published.member_content_revision_id),eligible:true,status:'PASSED',reasonCodes:[],canonicalContract:contract}};
    }
    // Existing non-trading-day digest remains separate from trading acceptance.
    ...
    const decisionPayload=buildCanonicalDecisionPayload(aiStrategyJson,importantNews,reportMode,marketBias,confScore,tradingDayInfo,learningConfidence);
    decisionPayload.correlation_id=correlationId;
    decisionPayload.safe_mode=safeMode;
    decisionPayload.started_at=startedAt;
    decisionPayload.duration_ms=Math.max(0,Date.now()-Date.parse(startedAt));
    decisionPayload.recovery_plan={attempt:Math.max(1,recoveryAttempt),reason_codes:Array.isArray(aiStrategyJson.content_reason_codes)?aiStrategyJson.content_reason_codes:[]};
    const snapshotId=await publishCanonicalDecisionSnapshot(supabase,todayDate,data.id,decisionPayload,correlationId,recoveryAttempt,log);
    const memberRevision=snapshotId
      ? await publishCanonicalMemberRevision(supabase,todayDate,data.id,snapshotId,aiStrategyJson,`member-content:${todayDate}:${snapshotId}:${String(decisionPayload.engine_version||VERSION)}`,String(decisionPayload.engine_version||VERSION),'generate-daily-report-v7',log)
      : {revisionId:null,eligible:false,status:'BLOCKED',reasonCodes:['CANONICAL_SNAPSHOT_MISSING'],canonicalContract:null};
    log('writeReport OK: '+data.id+' snapshot_id='+(snapshotId||'pending_migration')+' member_revision_id='+(memberRevision.revisionId||'blocked'));
    return{reportId:data.id,snapshotId,memberRevision};
  }catch(e){log('writeReport exc: '+(e instanceof Error?e.message:String(e)));throw e}
}
...
async function verifyReportExists(supabase:RuntimeClient,todayDate:string,log:(m:string)=>void):Promise<{reportId:string;aiJson:Record<string,unknown>|null}|null>{
...
async function readGeneratedPublication(supabase:RuntimeClient,report:{reportId:string;aiJson:Record<string,unknown>|null},reportDate:string,newsCount:number){
  const row={id:report.reportId,report_date:reportDate,ai_strategy_json:report.aiJson||{}};
  const evidence=await fetchPublishedDeliveryEvidence(supabase,row);
  const marketGate=evaluateMarketReportGate(row.ai_strategy_json,reportDate);
  const premiumGate=evaluatePremiumContentGate(row.ai_strategy_json,newsCount);
  const publication=evaluatePublishedMarketDelivery(row,evidence.snapshot,evidence.member,marketGate,{
    todayDate:reportDate,publicationRun:evidence.publicationRun,premiumEligible:premiumGate.eligible,
  });
  return{...evidence,marketGate,premiumGate,publication};
}
...
function buildResearchSectorRotationStatus(todayDate:string,scoreDate:string,sectorData:SectorRotationRow[],marketData:MarketIndicator[]):Record<string,unknown>{
  const status=buildSectorRotationStatus(todayDate,scoreDate,sectorData,marketData);
  const lineage=readSectorEvidenceLineage(sectorData[0]);
  if(!lineage)return status;
  return {...status,source:lineage.source,evidence_origin:lineage.origin,lineage,
    warning:'上一交易日正式類股輪動資料缺失；此為由權威收盤批次重建的研究脈絡，不是歷史正式分數。'};
}
...
Deno.serve(async (req:Request)=>{
  const reqStart=Date.now();const reqStartedAt=new Date(reqStart).toISOString();const requestedCorrelationId=req.headers.get('x-correlation-id')||'';const correlationId=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestedCorrelationId)?requestedCorrelationId:crypto.randomUUID();const logs:string[]=[];const log=function(m:string){const ts=new Date().toISOString().slice(11,19);logs.push('['+ts+'] '+m);console.log('[V9.4:'+ts+'] ['+correlationId.slice(0,8)+'] '+m)};
  ...
  let activeInputRun:ResearchInputRun|undefined;
  let inputRunClient:RuntimeClient|undefined;

  try{
    const auth=await authorizeInternalRequest(req.headers,internalCredentialsFromEnv());
    if(!auth.ok){log(auth.error_code||'INTERNAL_AUTH_INVALID');return corsResponse({success:false,error:auth.error_code,error_code:auth.error_code,version:VERSION},401)}

    let body:Record<string,unknown>={};try{body=await req.json()}catch{log('BODY_PARSE_FAILED')}
    let skipOpenAI=body?.skip_openai===true||body?.fast_mode===true;
    ...
    let budgetForcedSkip=false;
    let runtimeOpenAIUsage:Record<string,unknown>|null=null;
    let runtimeCostUsageId:string|null=null;
    let costGuard:Record<string,unknown>={policy_version:RUNTIME_QUALITY_POLICY.version,allowed:true,reason_codes:[]};
    let runtimePolicyIntegrity:Record<string,unknown>={policy_version:RUNTIME_QUALITY_POLICY.version,aligned:false};
    log('START V9.4 skip_openai='+skipOpenAI+' run_v10_debug_agents='+runV10DebugAgents+' quality_retry='+qualityRetry+' recovery_attempt='+recoveryAttempt);
    timer.mark('START');
    ...
    const supabase=createClient<RuntimeDatabase>(supabaseUrl,serviceRoleKey,{auth:{autoRefreshToken:false,persistSession:false}});
    inputRunClient=supabase;
    timer.mark('SUPABASE_CLIENT_READY');

    if(body?.mode==='canonical_member_recovery'){
      const reportDate=String(body.report_date||'');
      const recoveryIdempotencyKey=String(body.recovery_idempotency_key||'');
      const recoveryRequestId=String(body.recovery_request_id||'');
      const recoveryActor=String(body.recovery_actor||'').trim();
      const recoveryReason=String(body.recovery_reason||'').trim();
      const safeRecovery=body.source==='ma-ops-safe-recovery'&&body.suppress_notifications===true
        &&recoveryActor.length>0&&recoveryReason.length>0
        &&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(recoveryRequestId);
      if(!safeRecovery||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(reportDate)||!recoveryIdempotencyKey){
        return corsResponse({success:false,error:'CANONICAL_MEMBER_RECOVERY_CONTRACT_INVALID',error_code:'CANONICAL_MEMBER_RECOVERY_CONTRACT_INVALID',version:VERSION},400);
      }
      const recovery=await recoverCanonicalMemberRevision(supabase,reportDate,recoveryIdempotencyKey,log);
      return corsResponse({
        success:recovery.eligible,
        report_date:reportDate,
        report_id:recovery.reportId,
        decision_snapshot_id:recovery.snapshotId,
        member_content_revision_id:recovery.revisionId,
        semantic_status:recovery.status,
        semantic_reason_codes:recovery.reasonCodes,
        notifications_sent:0,
        checkpoints_replayed:0,
        closing_replayed:false,
        learning_replayed:false,
        version:VERSION,
      },recovery.eligible?200:409);
    }

    const todayDate=getTaipeiDateString();const dow=getTaipeiDayOfWeek();log('TODAY='+todayDate+' DOW='+dow);
    const dateError=currentResearchDateError(body,todayDate);
    if(dateError)return corsResponse({success:false,error:dateError,error_code:dateError,version:VERSION},400);
    ...
    try{
      const policyResult=await supabase.rpc('get_active_runtime_quality_policy_v1');
      const{data,error}=safeUnwrap<Record<string,unknown>>(policyResult,log,'getActiveRuntimeQualityPolicy');
      if(error||!data)throw new Error(error||'active runtime policy missing');
      const aligned=String(data.policy_version||'')===RUNTIME_QUALITY_POLICY.version
        && Number(data.premium_publish_min)===RUNTIME_QUALITY_POLICY.premium_publish_min
        && Number(data.member_value_min)===RUNTIME_QUALITY_POLICY.member_value_min
        && Number(data.auto_repair_min)===RUNTIME_QUALITY_POLICY.auto_repair_min;
      runtimePolicyIntegrity={policy_version:String(data.policy_version||''),aligned,checked_at:new Date().toISOString()};
      if(!aligned){skipOpenAI=true;budgetForcedSkip=true;costGuard={policy_version:String(data.policy_version||''),allowed:false,safe_mode:true,reason_codes:['runtime_policy_version_mismatch']};log('RUNTIME_POLICY_MISMATCH_FAIL_CLOSED');}
    }catch(error){
      skipOpenAI=true;budgetForcedSkip=true;
      runtimePolicyIntegrity={policy_version:RUNTIME_QUALITY_POLICY.version,aligned:false,error:'runtime_policy_unavailable',checked_at:new Date().toISOString()};
      costGuard={policy_version:RUNTIME_QUALITY_POLICY.version,allowed:false,safe_mode:true,reason_codes:['runtime_policy_unavailable']};
      log('RUNTIME_POLICY_UNAVAILABLE_FAIL_CLOSED '+(error instanceof Error?error.message:String(error)));
    }
    ...
    if(!tradingDayInfo.is_trading_day){
      ...
      marketClosedReport.runtime_policy_integrity=runtimePolicyIntegrity;
      ...
      const writeResult=await writeReport(supabase,todayDate,marketClosedReport,'休市',null,REPORT_MODE_NON_TRADING,[],[],log,tradingDayInfo,undefined,'',null,correlationId,Math.max(1,recoveryAttempt),reqStartedAt);
      ...
      return corsResponse({success:true,message:'Market closed',report_date:todayDate,report_id:writeResult.reportId,correlation_id:correlationId,is_trading_day:false,market_closed:true,holiday_name:tradingDayInfo.holiday_name,market_bias:'休市',confidence_score:null,report_mode:REPORT_MODE_NON_TRADING,duration_ms:durationMs,version:VERSION,logs},200);
    }
    ...
    if(!skipOpenAI){
      try{
        ...
        const{data,error}=safeUnwrap<Record<string,unknown>>(budgetResult,log,'checkRuntimeCostBudget');
        if(error||!data)throw new Error(error||'empty cost guard response');
        costGuard=data;
        if(data.allowed===false){skipOpenAI=true;budgetForcedSkip=true;log('OPENAI_BUDGET_GUARD_BLOCKED reasons='+JSON.stringify(data.reason_codes||[]));}
      }catch(error){
        skipOpenAI=true;budgetForcedSkip=true;
        costGuard={policy_version:RUNTIME_QUALITY_POLICY.version,allowed:false,safe_mode:true,reason_codes:['cost_guard_unavailable']};
        log('OPENAI_BUDGET_GUARD_FAIL_CLOSED '+(error instanceof Error?error.message:String(error)));
      }
    }
    ...
    const rawDatesFetch=(async function(){try{const [canonicalResult,legacyResult]=await Promise.all([supabase.from('market_quotes').select('symbol,captured_at,value,change_percent').lte('captured_at',new Date().toISOString()).order('captured_at',{ascending:false}).limit(60),supabase.from('market_data').select('symbol,captured_at,value,change_percent').lte('captured_at',new Date().toISOString()).order('captured_at',{ascending:false}).limit(60)]);const canonical=safeUnwrap<Record<string,unknown>[]>(canonicalResult,log,'rawCanonicalForDates');const legacy=safeUnwrap<Record<string,unknown>[]>(legacyResult,log,'rawLegacyForDates');const rows=mergeCanonicalAndLegacyMarketRows(canonical.data||[],legacy.data||[]);return normalizeMarketDataRows(rows).marketData.map(function(row){return{symbol:row.symbol,captured_at:row.updatedAt};});}catch{log('rawForDates fetch failed');return [] as Record<string,unknown>[];}})();
    const [marketSettled,newsSettled,sectorSettled,rawDatesSettled,previousReportSettled,recentReportsSettled]=await Promise.allSettled([
      withTimeout(fetchMarketData(supabase,log),5000,'market_data_query',log,{marketData:[] as MarketIndicator[],latestDataTime:null as Date|null,isStale:true,dataCount:0,rawDataCount:0,invalidNumericSources:[] as string[]}),
    ...
    const staleCoreSources=detectStaleCoreMarketData(marketData,dates);const unavailableSources=detectUnavailableMarketData(marketData,dates);
    const researchMarketData=filterFreshMarketIndicators(marketData,dates);const excludedResearchSymbols=marketData.filter((row)=>!researchMarketData.includes(row)).map((row)=>row.symbol);
    if(excludedResearchSymbols.length>0)log('RESEARCH_STALE_INPUT_FILTER excluded='+Array.from(new Set(excludedResearchSymbols)).join('|'));
    const twStatus=checkTWCoreStatus(marketData,log);const mvpStatus=checkMVPStatus(researchMarketData,log);
    const requiredEvidenceAvailable=!twStatus.dataInsufficient&&!mvpStatus.mvpInsufficient;
    const missingSources:string[]=[];if(dataCount===0)missingSources.push('market_data');if(newsData.length===0)missingSources.push(newsFetch.newsData.length>0?'market_news:no_verified_relevant_items':'market_news');if(sectorData.length===0)missingSources.push('sector_rotation_scores:'+sectorRotationReferenceDate);if(rawDataForDates.length===0)missingSources.push('market_data_dates');for(const invalid of marketFetch.invalidNumericSources)missingSources.push('invalid_numeric_market_data:'+invalid);if(twStatus.dataInsufficient)missingSources.push('required_tw_market_evidence');if(mvpStatus.mvpInsufficient)missingSources.push('required_us_market_evidence');for(const stale of staleCoreSources)missingSources.push('stale_market_data:'+stale);for(const unavailable of unavailableSources)missingSources.push('unavailable_market_data:'+unavailable);
    const uniqueMissingSources=Array.from(new Set(missingSources));missingSources.splice(0,missingSources.length,...uniqueMissingSources);
    const dataQuality=missingSources.length===0?'complete':'degraded';
    log('MARKET_DATA valid='+dataCount+' raw='+marketFetch.rawDataCount+' invalid_numeric='+marketFetch.invalidNumericSources.length);log('NEWS raw='+newsFetch.newsData.length+' verified='+newsData.length+' rejected='+newsReview.rejected.length);log('SECTOR_ROTATION rows='+sectorData.length);log('DATA_QUALITY '+dataQuality+' missing_sources='+(missingSources.join(',')||'none'));
    ...
    const learningInputs=await Promise.all([
    ...
    const inputFingerprint=await researchInputFingerprint({
      ...
      quality_policy:{...RUNTIME_QUALITY_POLICY,skip_openai:skipOpenAI,run_v10_debug_agents:runV10DebugAgents,policy_aligned:runtimePolicyIntegrity.aligned},
      sources:{market:marketData,news:newsData,sectors:sectorData,missing_sources:missingSources,learning:learningInputs.map((result)=>result.data)},
      previous_revision:{
        previous:buildV10PreviousValidation(previousReport),
        previous_daily_sentence:canonicalText(canonicalRecord(previousReport?.ai_strategy_json).today_quote,canonicalRecord(canonicalRecord(previousReport?.ai_strategy_json).v8_daily_sentence).sentence),
        // Only these fields feed the repeat-penalty engine. Report updated_at,
        // execution logs and unrelated runtime overlays are not input changes.
        recent:recentReportsForUniverse.map((report)=>({report_date:report.report_date,symbols:extractV10HistoricalSymbols(report)})),
      },
    });
    const claimed=await supabase.rpc('claim_research_input_v1',{p_report_date:todayDate,p_fingerprint:inputFingerprint,
      p_correlation_id:correlationId,p_engine_version:VERSION+':'+RESEARCH_PIPELINE_VERSION,
      p_trigger:body.recovery_request_id||body.recovery_actor?'manual_recovery':canonicalText(body.source,body.mode,'scheduled'),
      p_manifest:{report_date:todayDate,source_version:sectorSourceVersion,engine_version:RESEARCH_PIPELINE_VERSION,
        policy_version:RUNTIME_QUALITY_POLICY.version,input_fingerprint:inputFingerprint,source_dates:dates,missing_sources:missingSources,
        market_count:marketData.length,news_count:newsData.length,sector_count:sectorData.length,
        sector_evidence_origin:sectorEvidenceLineage?.origin??'PUBLISHED_SECTOR_ROTATION',
        sector_evidence_lineage:sectorEvidenceLineage},
    });
    if(claimed.error)throw new Error('RESEARCH_INPUT_CLAIM_UNAVAILABLE: '+claimed.error.message);
    const claim=canonicalRecord(claimed.data);
    if(claim.status!=='ACQUIRED'){
      const cached=canonicalRecord(claim.result);
      if(claim.status==='REUSED'&&claim.outcome==='SUCCEEDED'){
        const current=await verifyReportExists(supabase,todayDate,log);
        const currentAi=canonicalRecord(current?.aiJson);
        if(!current?.reportId || current.reportId!==cached.report_id || currentAi.revision_id!==cached.decision_snapshot_id){
          return corsResponse({success:false,error_code:'CACHED_REVISION_SUPERSEDED',report_date:todayDate,reused:true},409);
        }
        const cachedRead=await readGeneratedPublication(supabase,current,todayDate,newsData.length);
        if(!cachedRead.publication.eligible)return corsResponse({success:false,error_code:'CACHED_PUBLICATION_NOT_READY',reason_codes:cachedRead.publication.reason_codes,report_date:todayDate,reused:true},409);
        // Publication is durable before lifecycle bookkeeping. A transport
        // failure after commit must resume bookkeeping, never re-run the AI.
        for(const [state,checkpoint] of [['REPORT_GENERATED','report_generation'],['EDITORIAL_APPROVED','editorial_gate']]){
          const advanced=await supabase.rpc('advance_trading_day_state_v1',{p_trading_date:todayDate,p_state:state,
            p_checkpoint:checkpoint,p_status:'SUCCEEDED',p_correlation_id:correlationId,
            p_metadata:{...cached,input_fingerprint:inputFingerprint,reused:true}});
          if(advanced.error)throw new Error('CACHED_PUBLICATION_STATE_ADVANCE_FAILED');
        }
        return corsResponse({...cached,success:true,reused:true,report_generated:true,report_eligible:true,
          report_status:'READY',recommendation_status:cachedRead.publication.projection.recommendation.status,
          premium_eligible:cachedRead.premiumGate.eligible,premium_reason_codes:cachedRead.premiumGate.reason_codes,
          input_fingerprint:inputFingerprint,correlation_id:correlationId},200);
      }
      const code=claim.status==='IN_PROGRESS'?'RESEARCH_IN_PROGRESS':claim.status==='BACKOFF'?'RESEARCH_BACKOFF':claim.status==='EXHAUSTED'?'RESEARCH_RETRY_EXHAUSTED':'RESEARCH_QUALITY_REJECTED_UNCHANGED';
      return corsResponse({success:false,error_code:code,report_date:todayDate,input_fingerprint:inputFingerprint,reused:claim.status==='REUSED',reason_codes:cached.reason_codes||[]},409);
    }
    activeInputRun={id:String(claim.run_id),fingerprint:inputFingerprint,attempt:Number(claim.attempt)};


    let dScore=calculateMarketDataScore(marketData);const guardrailPreview=applyBiasGuardrails(marketData,dScore.baseScore,dates);if(guardrailPreview.shouldDowngrade){log('BIAS_GUARDRAIL adjusted_score='+guardrailPreview.adjustedScore+' risk='+(guardrailPreview.riskSignals.join('|')||'none')+' stale='+(guardrailPreview.staleSignals.join('|')||'none'));dScore={...dScore,baseScore:guardrailPreview.adjustedScore,riskReasons:Array.from(new Set([...dScore.riskReasons,...guardrailPreview.riskSignals,...guardrailPreview.staleSignals])),details:{...dScore.details,bias_guardrail_original_score:dScore.baseScore,bias_guardrail_adjusted_score:guardrailPreview.adjustedScore}};}
    ...
    const deterministicJson=buildDeterministicAIStrategyJson(researchMarketData,newsData,todayDate,dates,dScore,twStatus,mvpStatus,reportMode,sectorData,log,confidenceResult);
    ...
    const detMemberNoteText=typeof deterministicJson.member_research_note==='string'?String(deterministicJson.member_research_note):buildMemberResearchNoteText(researchMarketData,todayDate,dates,dScore,marketBiasDet,confScoreDet,Array.isArray(deterministicJson.today_beneficiary_stocks)?deterministicJson.today_beneficiary_stocks as Record<string,unknown>[]:[],reportMode,log);
    ...
    if(skipOpenAI){
      ...
      if(openAiKey){
        log('OPENAI_MODE');
        const strongSectors=sectorData.filter(isStrongSectorRotation).slice(0,3);
        const sectorContextSummary=strongSectors.length>0?strongSectors.map(function(s){const lineage=readSectorEvidenceLineage(s);const reference=lineage
          ?`${SECTOR_RECONSTRUCTION_SOURCE}:batch:${lineage.batch_id}:${s.sector}@${s.score_date}`
          :'sector_rotation_scores.'+s.sector;
          return 'evidence_ref='+reference+' | '+s.sector+'(輪動分數='+s.rotation_score+(lineage?'，權威收盤批次重建研究值':'')+')';
        }).join('；'):'上一交易日類股輪動資料缺失或無強勢族群';
        ...
        const sysPrompt=buildOpenAISystemPrompt()+'\n'+OPENAI_EVIDENCE_GUARDRAILS+'\n'+OPENAI_OUTPUT_ABSTENTION_RULES;const userPrompt=buildOpenAIUserPrompt(researchMarketData,newsData,todayDate,dates,sectorContextSummary,allowedCandidateRows);
        ...
        if(openAiResult){
          ...
          runtimeOpenAIUsage=openAiResult.__runtime_openai_usage&&typeof openAiResult.__runtime_openai_usage==='object'&&!Array.isArray(openAiResult.__runtime_openai_usage)?openAiResult.__runtime_openai_usage as Record<string,unknown>:null;
          delete openAiResult.__runtime_openai_usage;
          ...
          const sectorResult=buildThreeTierBeneficiaryStocks(researchMarketData,sectorData,dScore,log);
          ...
          if(!v8ContractHasUsableData(sanitizedV8)){
            const bridgeV8=buildV8ContractFromMemberNoteV2(openAiResult.member_research_note_v2,openAiResult.beneficiary_stocks as Record<string,unknown>[],researchMarketData,sectorData);
    ...
    if(!v8ContractHasUsableData(finalV8)){
      const bridgeV8=buildV8ContractFromMemberNoteV2(aiStrategyJson.member_research_note_v2,aiStrategyJson.beneficiary_stocks as Record<string,unknown>[],researchMarketData,sectorData);
    ...
    aiStrategyJson.sector_rotation_status=buildResearchSectorRotationStatus(todayDate,sectorRotationReferenceDate,sectorData,marketData);
    ...
    aiStrategyJson=applyMemberResearchNoteV2Aliases(aiStrategyJson);
    aiStrategyJson=limitBeneficiaryStockCounts(aiStrategyJson);
    if(!requiredEvidenceAvailable)aiStrategyJson=failClosedForMissingRequiredEvidence(aiStrategyJson,missingSources,log);
    ...
    aiStrategyJson.runtime_cost_guard=costGuard;
    aiStrategyJson.runtime_policy_integrity=runtimePolicyIntegrity;
    ...
    const writeResult=await writeReport(supabase,todayDate,aiStrategyJson,marketBias,rawConfidenceScore,reportMode,marketData,newsData,log,tradingDayInfo,dates,previousDailySentence,learningConfidence,correlationId,Math.max(1,recoveryAttempt),reqStartedAt,activeInputRun);
    ...
    if(runtimeOpenAIUsage){
      runtimeCostUsageId=await recordRuntimeOpenAIUsage(supabase,todayDate,correlationId,runtimeOpenAIUsage,'succeeded',log);
    }else if(budgetForcedSkip){
      runtimeCostUsageId=await recordRuntimeOpenAIUsage(supabase,todayDate,correlationId,null,'skipped_budget',log);
    }
    if(runtimeCostUsageId){
      const pipelineCostResult=await supabase.from('pipeline_runs').update({cost_usage_id:runtimeCostUsageId}).eq('idempotency_key','production-report:'+todayDate+':PREMARKET');
      if(pipelineCostResult.error)log('PIPELINE_COST_LINK_DEGRADED '+pipelineCostResult.error.message);
    }
    ...
    const verifiedRead=await readGeneratedPublication(supabase,verified,todayDate,newsData.length);
    const verifiedGate=verifiedRead.premiumGate;
    const exactReceipt=verified.reportId===writeResult.reportId&&verifiedRead.snapshot?.id===writeResult.snapshotId
      &&verifiedRead.member?.id===writeResult.memberRevision.revisionId;
    const deliveryEligible=verifiedRead.publication.eligible&&exactReceipt;
    const verifiedMarketGate={...verifiedRead.marketGate,
      report_status:deliveryEligible?'READY':verifiedRead.marketGate.report_status,
      recommendation_status:verifiedRead.publication.projection.recommendation.status,
      content_score:deliveryEligible?presentNumber(verifiedRead.snapshot?.content_score):null};
    const deliveryReasonCodes=Array.from(new Set([...verifiedRead.publication.reason_codes,
      ...(!exactReceipt?['PUBLISHED_REVISION_SUPERSEDED']:[])]));
    if(deliveryEligible){
      const reportState=await supabase.rpc('advance_trading_day_state_v1',{p_trading_date:todayDate,p_state:'REPORT_GENERATED',p_checkpoint:'report_generation',p_status:'SUCCEEDED',p_correlation_id:correlationId,p_metadata:{report_id:verified.reportId,decision_snapshot_id:writeResult.snapshotId,input_fingerprint:activeInputRun?.fingerprint||'',output_fingerprint:String(writeResult.snapshotId||'')}});
    ...
    return corsResponse({success:deliveryEligible,message:deliveryEligible?'Report generated and approved (V9.5)':'Report generated but delivery gate blocked',error:deliveryEligible?undefined:'DELIVERY_GATE_BLOCKED',error_code:deliveryEligible?undefined:'DELIVERY_GATE_BLOCKED',report_generated:true,report_date:todayDate,report_id:verified.reportId,decision_snapshot_id:writeResult.snapshotId,member_content_revision_id:writeResult.memberRevision.revisionId,semantic_status:writeResult.memberRevision.status,correlation_id:correlationId,report_eligible:deliveryEligible,report_status:verifiedMarketGate.report_status,recommendation_status:verifiedMarketGate.recommendation_status,premium_eligible:verifiedGate.eligible,premium_content_score:verifiedGate.content_score,premium_decision_mode:verifiedGate.decision_mode,premium_reason_codes:verifiedGate.reason_codes,report_reason_codes:writeResult.snapshotId?deliveryReasonCodes:Array.from(new Set([...deliveryReasonCodes,'editorial_snapshot_missing'])),recovery_attempt:recoveryAttempt,cost_guard:costGuard,cost_usage_id:runtimeCostUsageId,is_trading_day:tradingDayInfo.is_trading_day,market_closed:tradingDayInfo.market_closed,holiday_name:tradingDayInfo.holiday_name,tw_core_date:dates.twCoreDate,us_global_date:dates.usGlobalDate,source:String(aiStrategyJson.build_method||'deterministic'),market_bias:marketBias,confidence_score:safeInteger(rawConfidenceScore,50),report_mode:reportMode,duration_ms:durationMs,version:VERSION,logs},deliveryEligible?200:409);
  }catch(err){
    const qualityFailure=err instanceof PublicationQualityError;
    const msg=err instanceof Error?err.message:String(err);
    const result={success:false,error_code:qualityFailure?'RESEARCH_QUALITY_REJECTED':'RESEARCH_EXECUTION_FAILED',
      reason_codes:qualityFailure?err.reasonCodes:[msg],correlation_id:correlationId,version:VERSION};
    if(activeInputRun&&inputRunClient){
      const finished=await inputRunClient.rpc('finish_research_input_v1',{p_run_id:activeInputRun.id,p_correlation_id:correlationId,
        p_outcome:qualityFailure?'DEGRADED':'FAILED',p_result:result,
        p_retry_after_seconds:qualityFailure||activeInputRun.attempt>=3?null:30*2**(activeInputRun.attempt-1)});
      if(finished.error)log('RESEARCH_OUTCOME_PERSISTENCE_FAILED '+finished.error.message);
    }
    log('FATAL: '+msg);
    return corsResponse({...result,logs},qualityFailure?409:500);
  }
});
