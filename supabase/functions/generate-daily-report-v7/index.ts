...
type V10CandidateCard={symbol:string;name:string;sector:string;industry:string;market_cap_bucket:string;liquidity_score:number;trigger_tags:string[];related_evidence:V10EvidenceReference[];historical_repeat_days:number;repeat_penalty:number;eligibility:boolean;excluded_reason:string};
...
const V10_CANDIDATE_METADATA:Record<string,{name:string;sector:string;industry:string;market_cap_bucket:string;liquidity_score:number;tags:string[]}>={
  '2330':{name:'台積電',sector:'半導體',industry:'晶圓代工',market_cap_bucket:'mega',liquidity_score:100,tags:['SEMICONDUCTOR','AI_SERVER','TSM_ADR','TAIEX']},
  '2454':{name:'聯發科',sector:'半導體',industry:'IC設計',market_cap_bucket:'large',liquidity_score:90,tags:['SEMICONDUCTOR','AI_SERVER']},
  '2382':{name:'廣達',sector:'電腦及週邊',industry:'AI伺服器',market_cap_bucket:'large',liquidity_score:88,tags:['AI_SERVER']},
  '3231':{name:'緯創',sector:'電腦及週邊',industry:'AI伺服器',market_cap_bucket:'large',liquidity_score:86,tags:['AI_SERVER']},
  '3661':{name:'世芯-KY',sector:'半導體',industry:'ASIC',market_cap_bucket:'mid',liquidity_score:72,tags:['AI_SERVER','SEMICONDUCTOR']},
  '3017':{name:'奇鋐',sector:'電子零組件',industry:'散熱',market_cap_bucket:'mid',liquidity_score:78,tags:['AI_SERVER']},
  '3324':{name:'雙鴻',sector:'電子零組件',industry:'散熱',market_cap_bucket:'mid',liquidity_score:70,tags:['AI_SERVER']},
  '6669':{name:'緯穎',sector:'電腦及週邊',industry:'AI伺服器',market_cap_bucket:'large',liquidity_score:76,tags:['AI_SERVER']},
  '3037':{name:'欣興',sector:'電子零組件',industry:'PCB',market_cap_bucket:'mid',liquidity_score:73,tags:['AI_SERVER','SEMICONDUCTOR']},
  '2308':{name:'台達電',sector:'電子零組件',industry:'電源/散熱',market_cap_bucket:'large',liquidity_score:92,tags:['AI_SERVER','POLICY']},
  '2881':{name:'富邦金',sector:'金融',industry:'金控',market_cap_bucket:'large',liquidity_score:86,tags:['FX','RATE','DXY','US10Y']},
  '2882':{name:'國泰金',sector:'金融',industry:'金控',market_cap_bucket:'large',liquidity_score:86,tags:['FX','RATE','DXY','US10Y']},
  '2891':{name:'中信金',sector:'金融',industry:'金控',market_cap_bucket:'large',liquidity_score:82,tags:['FX','RATE','DXY','US10Y']},
  '2610':{name:'華航',sector:'航運',industry:'航空',market_cap_bucket:'mid',liquidity_score:74,tags:['FX','OIL']},
  '2615':{name:'萬海',sector:'航運',industry:'貨櫃航運',market_cap_bucket:'mid',liquidity_score:68,tags:['OIL','GEOPOLITICS']},
  '2603':{name:'長榮',sector:'航運',industry:'貨櫃航運',market_cap_bucket:'large',liquidity_score:79,tags:['OIL','GEOPOLITICS']},
  '1301':{name:'台塑',sector:'塑化',industry:'石化',market_cap_bucket:'large',liquidity_score:78,tags:['OIL']},
  '1303':{name:'南亞',sector:'塑化',industry:'塑化/電子材料',market_cap_bucket:'large',liquidity_score:75,tags:['OIL','SEMICONDUCTOR']},
  '6505':{name:'台塑化',sector:'塑化',industry:'油品',market_cap_bucket:'large',liquidity_score:72,tags:['OIL']},
  '2345':{name:'智邦',sector:'通信網路',industry:'網通',market_cap_bucket:'mid',liquidity_score:70,tags:['AI_SERVER','EARNINGS']},
  '3045':{name:'台灣大',sector:'通信網路',industry:'電信',market_cap_bucket:'large',liquidity_score:66,tags:['DEFENSIVE','RATE']},
  '2412':{name:'中華電',sector:'通信網路',industry:'電信',market_cap_bucket:'large',liquidity_score:80,tags:['DEFENSIVE','RATE']}
};
function v10PrimaryEventTags(marketContext:Record<string,unknown>,normalizedNews:Record<string,unknown>[]):string[]{const tags=new Set<string>();const primary=(marketContext.primary_event&&typeof marketContext.primary_event==='object'?marketContext.primary_event:{} ) as Record<string,unknown>;const sym=String(primary.symbol??'').toUpperCase();const event=String(primary.event_type??'').toUpperCase();if(event)tags.add(event);if(sym){if(['SOX','TSM','NVDA','2330'].includes(sym))tags.add('SEMICONDUCTOR');if(['NVDA','MSFT','META','AMZN'].includes(sym))tags.add('AI_SERVER');if(['VIX'].includes(sym))tags.add('RISK');if(['DXY'].includes(sym))tags.add('FX');if(['US10Y'].includes(sym))tags.add('RATE');if(['CRUDE OIL'].includes(sym))tags.add('OIL');if(['TAIEX','TXF'].includes(sym))tags.add('TAIEX');tags.add(sym);}for(const n of normalizedNews.slice(0,3)){const et=String(n.event_type??'').toUpperCase();if(et)tags.add(et);}if(tags.size===0)tags.add('SEMICONDUCTOR');return Array.from(tags);}
...
function calculateRepeatPenalty(symbol:string,recentReports:Record<string,unknown>[]):{historical_repeat_days:number;repeat_penalty:number}{let repeat=0;for(const report of recentReports.slice(0,10)){if(extractV10HistoricalSymbols(report).includes(symbol))repeat++;}const last3=recentReports.slice(0,3).filter((r)=>extractV10HistoricalSymbols(r).includes(symbol)).length;const penalty=Math.min(100,repeat*8+last3*12);return {historical_repeat_days:repeat,repeat_penalty:penalty};}
