...
// Preserve the deployed fresh-input filter; no stale quote may enter research.
export function filterFreshMarketIndicators<T extends { symbol: unknown; updatedAt: unknown }>(
  rows: T[], dates?: MarketFreshnessDates, nowMs = Date.now(),
): T[] {
  return rows.filter((row) => !isMarketIndicatorStale(String(row.updatedAt || ''), String(row.symbol || ''), dates, nowMs));
}
