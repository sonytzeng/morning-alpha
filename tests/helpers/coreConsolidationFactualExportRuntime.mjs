// Fresh 9/30 factual-market replay; no old business rows are adopted.
...
export const REPLAY_SCOPE_CONTRACTS = Object.freeze({
  'ma-consolidation-v1-20260909200000': Object.freeze({ version: 4, origin: 'http://127.0.0.1:55501', subnet: '172.22.0.0/16' }),
});
...
export function validateReplayConfiguration(config, environment = process.env) {
  ...
  assert.equal(config.scope, 'ma-consolidation-v1-20260909200000');
  assert.equal(config.report_date, '2026-09-30'); assert.equal(config.warmup_date, '2026-09-29');
  ...
  assert.equal(config.auth_fixture.provenance, 'FRESH_SCOPE_IDENTITY_WITH_AUTHORIZED_DEDICATED_VM_KEY2');
...
export function checkPinnedReplayInputs(config, repo) {
  ...
  const predecessorBytes = readExactFixturePreimage(FACTUAL_INPUT_PREDECESSOR.path, readFileSync(resolve(repo, FACTUAL_INPUT_PREDECESSOR.path)));
  validateDateOnlyProviderFixture(fixture, JSON.parse(predecessorBytes));
  assert.equal(sha256(predecessorBytes), FACTUAL_INPUT_PREDECESSOR.sha256);
...
export function readNewLocalCredentials(config) {
  ...
  assert.equal(data.scope, config.scope); assert.equal(data.provenance, 'FRESH_SCOPE_IDENTITY_WITH_AUTHORIZED_DEDICATED_VM_KEY2');
...
export function createReplayJournal(config) {
  ...
  const finish = status => {
    const result = { schema_version: 'CORE_CONSOLIDATION_FULL_CHAIN_RESULT_V1', run_id: runId, scope: config.scope,
      ...
      handler_count: 13, content_os_scope: 'ACTUAL_PUBLIC_EXPORT_PROJECTION_NOT_EXTERNAL_CONSUMER_DELIVERY',
