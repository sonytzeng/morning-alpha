// Independent 13-handler successor. Exact sealed predecessor: tests/helpers/coreConsolidationReplayRuntime.mjs
// SHA256 973000439d48b9341aa246d90aaa00198b70cf535889f715a5b586d4c60aa178; old file is never edited or executed against a reused scope.
...
export const REPLAY_SCOPE_CONTRACTS = Object.freeze({
  'ma-consolidation-v1-20260909183000': Object.freeze({ version: 3, origin: 'http://127.0.0.1:55491', subnet: '172.21.0.0/16' }),
});
...
export function validateReplayConfiguration(config, environment = process.env) {
  ...
  assert.equal(config.scope, 'ma-consolidation-v1-20260909183000');
  assert.equal(config.report_date, '2026-09-23'); assert.equal(config.warmup_date, '2026-09-22');
  ...
  assert.equal(config.auth_fixture.provenance, 'GENERATED_IN_THIS_ISOLATED_STACK');
  assert.equal(config.auth_fixture.new_synthetic_users, 1);
  assert.equal(config.auth_fixture.existing_scope_credentials_read, false);
  ...
  assert.notEqual(config.clock.configuration.bootId, '2879f43f-0819-4a08-88dc-195166474350', 'Never reuse the prior shared VM');
