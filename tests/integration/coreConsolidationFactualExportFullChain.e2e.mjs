// Independent fresh 9/30 source-fix run; executed 9/23 driver remains untouched.
...
import { validateReplayConfiguration, checkPinnedReplayInputs, readNewLocalCredentials, guardedLocalFetch,
  ...
  createReplayJournal, sha256, object, canonicalNumeric, REPLAY_FUNCTIONS } from '../helpers/coreConsolidationFactualExportRuntime.mjs';
...
export async function runCoreConsolidationPublicExportFullChain(configPath) {
  const supplied = JSON.parse(readFileSync(configPath, 'utf8'));
  // This successor is fresh-only. Never adopt an original 96-record PASS or old
  // failure as a new source run, and never reach the legacy continuation branches.
  assert.equal(supplied.schema_version, 'CORE_CONSOLIDATION_FULL_CHAIN_CONFIG_V4');
  assert.equal(supplied.scope, 'ma-consolidation-v1-20260909200000');
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  assert.equal(process.argv[2], '--config', 'Usage: node --experimental-strip-types tests/integration/coreConsolidationPublicExportFullChain.e2e.mjs --config /private/tmp/<fresh-scope>/replay-config.json');
  await runCoreConsolidationPublicExportFullChain(resolve(process.argv[3]));
}
