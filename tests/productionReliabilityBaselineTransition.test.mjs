...
test('reviewed parity baseline preserves the reliability transition and reconstructs all predecessors', () => {
  ...
  const result = verify();
  assert.equal(result.reviewedBaselineTransition.transition_id, 'MORNING_ALPHA_PRECHECK_PRODUCTION_PARITY_20260916');
  assert.equal(result.reviewedBaselineTransition.predecessor_integrity_id, 'MORNING_ALPHA_PRODUCTION_PARITY_20260916');
  ...
  assert.equal(hash(result.reviewedBaselinePredecessor.reviewedBaselinePredecessor.reviewedBaselinePredecessorReadSource(ENTRY)),
    'd4cda72931e160b6e07457f83f916c5390e0123bf497e81d33de8c37b42994bc');
  assert.equal(hash(result.reviewedBaselinePredecessor.reviewedBaselinePredecessor.reviewedBaselinePredecessorReadSource(PIPELINE_TEST)),
